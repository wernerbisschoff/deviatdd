...
<prerequisites>
<required_scripts_path>The script is colocated with SKILL.md inside the skill directory, NOT on $PATH. Always reference it as <SKILL_DIR>/deviate-tasks.sh.</required_scripts_path>
<failure_mode>ERROR: Operational orchestrator not found at <SKILL_DIR>/deviate-tasks.sh. Stop execution immediately and notify the host user.</failure_mode>
</prerequisites>
...
1. `cd` into the worktree (using the `worktree_full` path from your context) and run the pre-script to validate `spec.md` has required sections and emit a JSON contract:
   ```
   <SKILL_DIR>/deviate-tasks.sh pre
   ```
...
8. Run the post-script to validate, commit (still inside the worktree from step 1):
   ```
   <SKILL_DIR>/deviate-tasks.sh post
   ```
