...
**IMPORTANT**: The script `deviate-context.sh` lives in this skill's directory (alongside `SKILL.md`) and is NOT on `PATH`. Always invoke it as `<SKILL_DIR>/deviate-context.sh`.
...
<prerequisites>
<required_scripts_path>The script is colocated with SKILL.md inside the skill directory, NOT on $PATH. Always reference it as <SKILL_DIR>/deviate-context.sh.</required_scripts_path>
<failure_mode>ERROR: Operational orchestrator not found at <SKILL_DIR>/deviate-context.sh. Terminate execution immediately.</failure_mode>
</prerequisites>
...
```bash
<SKILL_DIR>/deviate-context.sh post "$PLAN_TARGET"
```
