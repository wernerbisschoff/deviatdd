#!/usr/bin/env bash
#
# deviate-context.sh - Orchestrator for the /deviate-context skill
#
# This script wraps operational concerns of agent context synchronization:
#   - Repository discovery (walk-up to .git)
#   - Spec directory discovery (most recently modified NNN-* under specs/)
#   - spec.md and constitution path resolution
#   - CLAUDE.md and AGENTS.md discovery
#   - Language/monorepo bounds assessment
#   - Context file validation
#   - Symlink integrity enforcement (AGENTS.md → CLAUDE.md)
#   - Atomic git commit
#
# Usage:
#   deviate-context.sh pre
#   deviate-context.sh post <MANIFEST_PATH> [--dry-run]
#
# The 'pre' subcommand discovers the spec directory, resolves context file
# paths, and emits a JSON contract. The 'post' subcommand reads the
# LLM-written execution manifest, validates context updates, stages files,
# and commits.
#
# Exit codes:
#   0   Success
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for the /deviate-context skill

SYNOPSIS
	$SCRIPT_NAME <pre|post> [OPTIONS]

DESCRIPTION
	Wraps operational concerns of agent context synchronization. The 'pre'
	subcommand discovers the spec directory, resolves context file paths,
	and emits a JSON contract. The 'post' subcommand reads the LLM-written
	execution manifest, validates context updates, stages files, and commits.

SUBCOMMANDS
	pre     Discover spec dir, resolve context paths, emit JSON contract
	post    Validate context updates, stage, commit
...
EXAMPLES
	...
	$SCRIPT_NAME post /tmp/deviate-context.XXX/execution-manifest.json
EOF
}
...
discover_spec_dir() {
	...
	SPEC_DIR="${latest#"${REPO_ROOT}"/}"
	SPEC_ABS_DIR="$latest"
	export SPEC_DIR SPEC_ABS_DIR
	return 0
}

# ── Context file path resolution ─────────────────────────────────────────
resolve_context_paths() {
	SPEC_MD_PATH="$SPEC_ABS_DIR/spec.md"
	CONSTITUTION_PATH="$REPO_ROOT/specs/constitution.md"
	CLAUDE_MD_PATH="$REPO_ROOT/CLAUDE.md"
	AGENTS_MD_PATH="$REPO_ROOT/AGENTS.md"
	COPILOT_INSTRUCTIONS_PATH="$REPO_ROOT/.github/agents/copilot-instructions.md"

	if [[ ! -f "$SPEC_MD_PATH" ]]; then
		log_err "spec.md not found at $SPEC_MD_PATH"
		return 1
	fi
	log_ok "spec.md found: $SPEC_MD_PATH"

	if [[ ! -f "$CONSTITUTION_PATH" ]]; then
		log_warn "constitution.md not found at $CONSTITUTION_PATH"
		CONSTITUTION_PATH=""
	fi

	export SPEC_MD_PATH CONSTITUTION_PATH CLAUDE_MD_PATH AGENTS_MD_PATH COPILOT_INSTRUCTIONS_PATH
	return 0
}

# ── Language bounds assessment ───────────────────────────────────────────
# Count major language footprints in the repo to determine multilang_mode.
assess_language_bounds() {
	local primary_languages=()
	local ext

	while IFS= read -r ext; do
		case "$ext" in
		py) primary_languages+=("Python") ;;
		js) primary_languages+=("JavaScript") ;;
		ts | tsx) primary_languages+=("TypeScript") ;;
		rs) primary_languages+=("Rust") ;;
		go) primary_languages+=("Go") ;;
		ex | exs) primary_languages+=("Elixir") ;;
		java) primary_languages+=("Java") ;;
		rb) primary_languages+=("Ruby") ;;
		php) primary_languages+=("PHP") ;;
		cs) primary_languages+=("C#") ;;
		swift) primary_languages+=("Swift") ;;
		kt) primary_languages+=("Kotlin") ;;
		esac
	done < <(cd "$REPO_ROOT" && find . -type f -name "*.py" -o -name "*.js" -o -name "*.ts" -o -name "*.tsx" -o -name "*.rs" -o -name "*.go" -o -name "*.ex" -o -name "*.exs" -o -name "*.java" -o -name "*.rb" -o -name "*.php" -o -name "*.cs" -o -name "*.swift" -o -name "*.kt" 2>/dev/null | sed -n 's/.*\.//p' | sort -u | head -10)

	# Deduplicate extensions
	local unique_langs=()
	local lang
	for lang in "${primary_languages[@]}"; do
		local found=false
		local u
		for u in "${unique_langs[@]}"; do
			[[ "$u" == "$lang" ]] && found=true && break
		done
		"$found" || unique_langs+=("$lang")
	done

	if [[ ${#unique_langs[@]} -ge 2 ]]; then
		MULTILANG_MODE="append"
	else
		MULTILANG_MODE="replace"
	fi

	if [[ ${#unique_langs[@]} -gt 0 ]]; then
		LANGUAGE="${unique_langs[0]}"
	else
		LANGUAGE="Unknown"
	fi

	export MULTILANG_MODE LANGUAGE
}

# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-context pre-phase starting..."
	log_info "  dry_run: $DRY_RUN"

	validate_repo
	log_ok "Repository validated: $REPO_ROOT"

	if ! discover_spec_dir; then
		log_err "Could not resolve spec directory"
		emit_json_contract \
			"status" "NO_SPEC" \
			"phase" "context" \
			"skill_dir" "$SKILL_DIR" \
	...
	log_ok "Spec directory: $SPEC_DIR"

	if ! resolve_context_paths; then
		log_err "Could not resolve context file paths"
		emit_json_contract \
			"status" "FAILURE" \
			"phase" "context" \
			"spec_dir" "$SPEC_DIR" \
	...
	log_ok "spec.md path: $SPEC_MD_PATH"
	log_ok "CLAUDE.md path: $CLAUDE_MD_PATH"

	assess_language_bounds
	log_ok "Language: $LANGUAGE"
	log_ok "Multilang mode: $MULTILANG_MODE"
	...
	local temp_dir
	temp_dir=$(create_temp_dir "deviate-context") || {
	...
	contract_json=$(jq -n \
		...
		--arg spec_md_path "$SPEC_MD_PATH" \
		--arg constitution_path "$CONSTITUTION_PATH" \
		--arg claude_md_path "$CLAUDE_MD_PATH" \
		--arg agents_md_path "$AGENTS_MD_PATH" \
		--arg copilot_instructions_path "$COPILOT_INSTRUCTIONS_PATH" \
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-context post-phase starting..."
	...
	# Re-discover spec dir and resolve paths from the repo
	discover_spec_dir || true
	resolve_context_paths || true

	# Dry-run: emit preview and stop
	if [[ "$DRY_RUN" == "true" ]]; then
		log_info "Dry run — no mutations will be made"
		printf '%s' "$manifest" | jq -n --argjson m "$manifest" \
			'{status:"DRY_RUN",manifest:$m}'
		exit 0
	fi

	cd "$REPO_ROOT"

	# Step 1: Symlink enforcement — AGENTS.md → CLAUDE.md
	if [[ -f "$CLAUDE_MD_PATH" ]]; then
		if [[ -L "$AGENTS_MD_PATH" ]]; then
			local current_target
			current_target=$(readlink "$AGENTS_MD_PATH")
			if [[ "$current_target" != "CLAUDE.md" ]]; then
				log_info "Fixing AGENTS.md symlink target: $current_target → CLAUDE.md"
				ln -sf "CLAUDE.md" "$AGENTS_MD_PATH"
			fi
		elif [[ -f "$AGENTS_MD_PATH" ]]; then
			log_info "AGENTS.md is a regular file — replacing with symlink to CLAUDE.md"
			rm -f "$AGENTS_MD_PATH"
			ln -sf "CLAUDE.md" "$AGENTS_MD_PATH"
		else
			log_info "Creating AGENTS.md symlink → CLAUDE.md"
			ln -sf "CLAUDE.md" "$AGENTS_MD_PATH"
		fi
		log_ok "AGENTS.md → CLAUDE.md symlink verified"
	else
		log_warn "CLAUDE.md not found — skipping symlink enforcement"
	fi
	...
	if [[ -f "$CLAUDE_MD_PATH" ]]; then
		update_files+=("CLAUDE.md")
	fi
	if [[ -L "$AGENTS_MD_PATH" ]] || [[ -f "$AGENTS_MD_PATH" ]]; then
		update_files+=("AGENTS.md")
	fi

	if [[ ${#update_files[@]} -eq 0 ]]; then
		log_err "No context files found to validate"
		echo '{"status":"FAILURE","reason":"NO_CONTEXT_FILES"}'
		exit 5
	fi

	log_ok "Context files to update: ${update_files[*]}"

	# Step 3: Stage files
	log_info "Staging context files..."
	local uf
	for uf in "${update_files[@]}"; do
		git add "$uf" 2>/dev/null || true
	done
	# Also stage spec files
	if [[ -n "${SPEC_DIR:-}" ]]; then
		git add "$SPEC_DIR/"*.md 2>/dev/null || true
	fi
	git add "specs/constitution.md" 2>/dev/null || true
	...
	if git commit --no-verify -m "$commit_subject" 2>/dev/null; then
		...
		emit_json_contract \
			...
			"spec_dir" "${SPEC_DIR:-}" \
			"next_action" "Context synchronization complete" \
			"timestamp" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
	else
