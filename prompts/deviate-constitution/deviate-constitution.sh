#!/usr/bin/env bash
#
# deviate-constitution.sh - Orchestrator for the /deviate-constitution skill
#
# This script wraps operational concerns of constitution generation:
#   - Git state gathering (repo_root, branch, timestamp)
#   - Constitution path discovery (specs/constitution.md)
#   - Test/lint command resolution from project manifests
#   - Constitution validation (file existence, section completeness)
#   - Atomic git commit
#
# Usage:
#   deviate-constitution.sh pre [--dry-run]
#   deviate-constitution.sh post <MANIFEST_PATH> [--dry-run]
#
# The 'pre' subcommand emits a JSON contract on stdout. The 'post' subcommand
# reads the LLM-written execution manifest, validates the constitution,
# stages files, and commits.
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
#   2   Pre-flight check failed (not a repo)
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for the /deviate-constitution skill

SYNOPSIS
	$SCRIPT_NAME <pre|post> [OPTIONS]

DESCRIPTION
	Wraps operational concerns of constitution generation. The 'pre'
	subcommand gathers git state and emits a JSON contract. The 'post'
	subcommand reads the LLM-written execution manifest, validates the
	constitution artifact, stages files, and commits.

SUBCOMMANDS
	pre     Gather git state, emit JSON contract
	post    Validate constitution, stage, commit
...
EXAMPLES
	...
	$SCRIPT_NAME post /tmp/deviate-constitution.XXX/execution-manifest.json
EOF
}
...
# ── Constitution path discovery ──────────────────────────────────────────
discover_constitution() {
	local constitution=""
	for candidate in "$REPO_ROOT/specs/constitution.md" "$REPO_ROOT/CONSTITUTION.md"; do
		if [[ -f "$candidate" ]]; then
			constitution="$candidate"
			break
		fi
	done
	CONSTITUTION_PATH="${constitution:-$REPO_ROOT/specs/constitution.md}"
	export CONSTITUTION_PATH
}

# ── Project-type detection → test/lint commands ─────────────────────────
resolve_commands() {
	...
	export TEST_COMMAND LINT_COMMAND
}

# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-constitution pre-phase starting..."
	...
	discover_constitution
	log_ok "Constitution path: $CONSTITUTION_PATH"
	...
	local temp_dir
	temp_dir=$(create_temp_dir "deviate-constitution") || {
	...
	# Determine if constitution already exists
	local action="created"
	[[ -f "$CONSTITUTION_PATH" ]] && action="modified"
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-constitution post-phase starting..."
	...
	if [[ "$DRY_RUN" == "true" ]]; then
		...
		printf '%s' "$manifest" | jq -n --argjson m "$manifest" \
			...
			'{status:"DRY_RUN",constitution_path:$constitution_path,manifest:$m}'
		exit 0
	fi

	cd "$REPO_ROOT"

	# Step 1: Validate constitution artifact
	if [[ ! -f "$CONSTITUTION_PATH" ]]; then
		log_err "Constitution not found at: $CONSTITUTION_PATH"
		echo '{"status":"FAILURE","reason":"CONSTITUTION_NOT_FOUND","path":"'"$CONSTITUTION_PATH"'"}'
		exit 5
	fi

	if [[ ! -s "$CONSTITUTION_PATH" ]]; then
		log_err "Constitution is empty at: $CONSTITUTION_PATH"
		echo '{"status":"FAILURE","reason":"CONSTITUTION_EMPTY","path":"'"$CONSTITUTION_PATH"'"}'
		exit 5
	fi
	log_ok "Constitution exists and is non-empty: $CONSTITUTION_PATH"
	...
	for section in "${required_sections[@]}"; do
		if ! grep -q "\[[0-9_]*${section}\]" "$CONSTITUTION_PATH"; then
			missing_sections+=("$section")
		fi
	done

	if [[ ${#missing_sections[@]} -gt 0 ]]; then
		log_warn "Constitution missing required sections: ${missing_sections[*]}"
	# Non-fatal — constitution can be minimal
	else
		log_ok "All required sections present"
	fi

	# Step 3: Stage files
	log_info "Staging constitution file..."
	local constitution_rel="${CONSTITUTION_PATH#"${REPO_ROOT}"/}"
	git add "$constitution_rel" 2>/dev/null || true
	...
	if git commit --no-verify -m "$commit_subject" 2>/dev/null; then
		...
		emit_json_contract \
			...
			"constitution_path" "$constitution_rel" \
			"next_action" "Proceed to explore/research/PRD phase as needed" \
			"timestamp" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
	else
