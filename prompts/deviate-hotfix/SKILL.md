...
**IMPORTANT**: The script `deviate-hotfix.sh` lives in this skill's directory (alongside `SKILL.md`) and is NOT on `PATH`. Always invoke it as `<SKILL_DIR>/deviate-hotfix.sh`.
...
<SKILL_DIR>/deviate-hotfix.sh pre
</step_0_run_pre_script>
...
```bash
<SKILL_DIR>/deviate-hotfix.sh post
```
