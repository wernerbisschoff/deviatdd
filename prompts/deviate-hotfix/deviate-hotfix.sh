...
#
# deviate-hotfix.sh - Orchestrator for the /deviate-hotfix (bug-fix planning)
#
# This script wraps the operational concerns of hotfix planning:
#   - Bug context discovery (issue tracking, git log, spec files)
#   - Source file identification matching bug keywords
#   - Task file generation scaffolding
#   - File staging and conventional commit
#
# Usage:
#   deviate-hotfix.sh pre    # Discover bug context, emit JSON contract
#   deviate-hotfix.sh post   # Stage tasks.md and commit
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
#   2   Pre-flight check failed
#   3   Bug discovery failed
#
set -euo pipefail
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for /deviate-hotfix bug-fix planning

SYNOPSIS
	$SCRIPT_NAME <pre|post>

SUBCOMMANDS
	pre     Discover bug context, emit JSON contract
	post    Stage tasks.md and commit
...
EXAMPLES
	$SCRIPT_NAME pre     # Discover bug context and emit contract
	$SCRIPT_NAME post    # Stage and commit tasks.md
EOF
}
...
# ── Discover bug context ────────────────────────────────────────────────
discover_bug_context() {
	local issue_id=""
	...
	# Priority 1: Look for issue files in spec directories
	local spec_issues
	spec_issues=$(find "$REPO_ROOT/specs" -name "*.md" -path "*/issues/*" 2>/dev/null | head -3 || true)
	if [[ -n "$spec_issues" ]]; then
		local first_issue
		first_issue=$(echo "$spec_issues" | head -1)
		issue_id=$(basename "$first_issue" .md | grep -oE '[0-9]+' | head -1 || echo "")
		bug_description=$(head -5 "$first_issue" 2>/dev/null | grep -oE '^#+\s+.*' | head -1 | sed 's/^#*\s*//' || echo "Bug fix required")
		if [[ -z "$issue_id" ]]; then
			issue_id="hotfix-$(date +%s)"
		fi
		log_ok "Found issue context: $first_issue"
	fi

	# Priority 2: Look at commit history for bug-related keywords
	if [[ -z "$issue_id" ]]; then
		local recent_commits
		recent_commits=$(git --no-pager log --oneline -5 --grep="bug\|fix\|hotfix\|issue" 2>/dev/null || true)
		if [[ -n "$recent_commits" ]]; then
			issue_id="BUG-$(date +%s)"
			bug_description="Bug fix inferred from recent commit history"
			log_info "No issue file found; inferred bug context from recent commits"
		fi
	fi

	# Priority 3: Use branch name as context
	if [[ -z "$issue_id" ]]; then
		issue_id="BRANCH-$(echo "$branch_name" | sha256sum | cut -c1-8)"
		bug_description="Bug fix on branch: $branch_name"
		log_info "No issue context found; using branch name"
	fi

	# Default fallback
	if [[ -z "$bug_description" ]]; then
		bug_description="Bug fix required"
	fi
...
# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-hotfix pre-phase starting..."

	validate_repo
	log_ok "Repository validated: $REPO_ROOT"

	discover_bug_context
	log_ok "Bug context discovered: $BUG_ISSUE_ID"
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-hotfix post-phase starting..."
	...
	# Stage tasks.md if it exists
	if [[ -f "$REPO_ROOT/tasks.md" ]]; then
		log_info "Staging tasks.md"
		git add "$REPO_ROOT/tasks.md"
	fi

	# Stage any spec files
	...
	# Commit
	local commit_subject="docs(hotfix): add hotfix task plan"
	if ! git commit -m "$commit_subject" -m "Issue: $BUG_ISSUE_ID"; then
