...
#
# deviate-prune.sh - Orchestrator for /deviate-prune (TDD PRUNE phase)
#
# This script wraps the operational concerns of the TDD PRUNE phase:
#   - Locate the active TDD task from tasks.md
#   - Identify the target test file to prune
#   - Validate the task exists in tasks.md
#   - Extract test_command and lint_command from constitution.md
#   - Emit JSON contract with task context
#   - Stage pruned test files and commit
#
# Usage:
#   deviate-prune.sh pre    # Discover task, identify target test file, emit JSON contract
#   deviate-prune.sh post   # Stage pruned test files and commit
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for /deviate-prune (TDD PRUNE phase)

SYNOPSIS
	$SCRIPT_NAME <pre|post>

SUBCOMMANDS
	pre     Discover active task, identify target test file, extract test/lint commands, emit JSON contract
	post    Stage pruned test files and commit
...
EXAMPLES
	...
	$SCRIPT_NAME post    # Stage and commit pruned test files
EOF
}
...
locate_active_task() {
	...
	while IFS= read -r candidate; do
		...
		# Look for in-progress tasks (GREEN phase just completed)
		local active_line
		...
		if [[ -n "$active_line" ]]; then
			...
			for ((i = from_line; i <= to_line; i++)); do
				...
				if echo "$line" | grep -qE 'Target_Test_File|Target_File|test_file'; then
					target_test_file=$(echo "$line" | sed -E 's/.*:\s*//' | xargs)
				elif echo "$line" | grep -qE 'Task_Type'; then
	...
	# Validate that the task exists in tasks.md
	if ! grep -qE "\[$task_id\]" "$tasks_file" 2>/dev/null; then
		log_err "Task $task_id not found in $tasks_file"
		echo '{"status":"FAILURE","reason":"TASK_NOT_FOUND","task_id":"'"$task_id"'","tasks_file":"'"$tasks_file"'"}'
		exit 3
	fi
	log_ok "Task $task_id validated in $tasks_file"

	# If no target_test_file found in task fields, try to derive from files_touched or project structure
	if [[ -z "$target_test_file" ]]; then
		# Look for test files matching the task scope
		if [[ -n "$files_touched" ]]; then
			while IFS= read -r f; do
				[[ -z "$f" ]] && continue
				# Check if the file is a test file
				if echo "$f" | grep -qE '(test|spec)\.(py|js|ts|ex|rs)$'; then
					target_test_file="$f"
					break
				fi
			done <<<"$files_touched"
...
# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-prune pre-phase starting..."
	...
	log_ok "Target test file: ${TARGET_TEST_FILE:-<not found — will prompt user>}"
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-prune post-phase starting..."
	...
	# Stage all tracked changes (pruned test files written by LLM)
	git add -u
	...
	local commit_subject="test(prune): optimize test suite for behavioral focus"
	if ! git commit -m "$commit_subject" -m "Phase: PRUNE"; then
