#!/usr/bin/env bash
#
# research.sh - Orchestrator for the /deviate-research (PHASE_RESEARCH) macro layer
#
# This script wraps operational concerns of the research workflow:
#   - Active feature resolution (from --feature argument, $RESEARCH_FEATURE_DIR, or latest epic)
#   - Upstream explore.md validation gate
#   - Allocation of design.md and data-model.md targets
#   - Constitution re-validation
#   - Pre-flight validation of design + data-model artifacts
#   - Atomic git commit
#
# Usage:
#   research.sh pre [--feature <feature-dir>]
#   research.sh post
#
# The 'pre' subcommand emits a JSON contract to stdout. The contract carries
# the absolute paths the orchestrator must use and the metadata inherited from
# the upstream 'explore' contract. The 'post' subcommand reads the
# LLM-written artifacts, validates them, commits them, and returns
# STATUS: SUCCESS.
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
#   2   Pre-flight check failed (not a repo, no active feature, constitution missing)
#   4   Upstream gate failed (explore.md missing or empty)
...
# Resolve the active feature directory. Order of precedence:
#   1. --feature <value> flag (or first positional arg) — accepts:
#        - an absolute directory path
#        - a specs-relative directory path
#        - a bare NNN epic id (e.g. "001") or NNN-slug (e.g. "001-foo")
#        - a path to an explore.md file (parent directory is used)
#   2. $RESEARCH_FEATURE_DIR env var
#   3. Most recently modified {NNN}-* directory under specs/
resolve_active_feature() {
	...
	if [ -n "$explicit" ]; then
		# 1a. Directory path (absolute or specs-relative)
		if [ -d "$explicit" ]; then
			(cd "$explicit" && pwd)
			return 0
		fi
		if [ -d "$specs_dir/$explicit" ]; then
			echo "$specs_dir/$explicit"
			return 0
		fi
		# 1b. Path to an explore.md file — use its parent directory
		if [ -f "$explicit" ] && [ "$(basename "$explicit")" = "explore.md" ]; then
			(cd "$(dirname "$explicit")" && pwd)
			return 0
		fi
		# 1c. Bare epic id (NNN) or NNN-slug — glob for the matching bucket
		if echo "$explicit" | grep -qE '^[0-9]{3}(-[a-z0-9][a-z0-9-]*)?$'; then
			local matches
			matches=$(ls -1d "$specs_dir"/${explicit}* 2>/dev/null | head -1 || true)
	...
	if [ -n "${RESEARCH_FEATURE_DIR:-}" ] && [ -d "${RESEARCH_FEATURE_DIR}" ]; then
		(cd "${RESEARCH_FEATURE_DIR}" && pwd)
		return 0
	fi

	# Fall back to the most recently modified NNN-* feature directory
	if [ -d "$specs_dir" ]; then
		local latest
...
# Derive the epic id (e.g. "001") from a feature slug like "001-foo".
# Falls back to the full basename when the slug does not match the NNN- pattern
# (e.g., legacy or externally-allocated buckets).
epic_id_from_slug() {
	local slug="$1"
	if echo "$slug" | grep -qE '^[0-9]{3}-'; then
		echo "$slug" | grep -oE '^[0-9]{3}'
	else
		echo "$slug"
	fi
}
...
cmd_pre() {
	...
	while [ $# -gt 0 ]; do
		...
		-h | --help)
			cat <<EOF
$SCRIPT_NAME pre [<epic> | --feature <value>]

Positional argument (or --feature value):
  <epic>     Epic id (e.g. "001"), NNN-slug (e.g. "001-foo"), absolute
             directory, specs-relative directory, or path to an explore.md
             file. When omitted, falls back to the RESEARCH_FEATURE_DIR env
             var (if set) or the most recently modified NNN-* directory
             under specs/.

Options:
  --feature <value>  Same as the positional argument.
...
EOF
		...
		--)
			...
			;;
		-*)
			log_err "Unknown pre-script argument: $1"
			exit 1
			;;
		*)
			# First positional argument is treated as the explicit feature
	...
	# Optional: light-touch extraction (same fields the explore pre-script reads)
	...
	local feature_abs
	feature_abs=$(resolve_active_feature "$explicit_feature" "$repo_root") || {
		echo '{"status":"FAILURE","reason":"NO_ACTIVE_FEATURE: no --feature flag, no $RESEARCH_FEATURE_DIR, and no specs/NNN-* directories exist"}'
		exit 2
	}
	local feature_dir_rel="${feature_abs#${repo_root}/}"
	local feature_slug
	feature_slug=$(basename "$feature_dir_rel")

	# Derive specs_directory (the parent of feature_dir)
	local specs_directory
	specs_directory=$(dirname "$feature_dir_rel")

	# Upstream gate — explore.md must exist and be non-empty
	local explore_md_abs="$feature_abs/explore.md"
	if [ ! -f "$explore_md_abs" ] || [ ! -s "$explore_md_abs" ]; then
		echo '{"status":"FAILURE","reason":"EXPLORE_MISSING: the explore skill must run and produce explore.md before research"}'
		exit 4
	fi

	# Allocate design + data-model targets
	local design_target_abs="$feature_abs/design.md"
	local design_target_rel="$feature_dir_rel/design.md"
	local data_model_target_abs="$feature_abs/data-model.md"
	local data_model_target_rel="$feature_dir_rel/data-model.md"

	# Derive the epic id from the feature slug. The epic id is the feature
	# bucket number (e.g. "001"), NOT an issue id — at this phase no issue
	# exists yet, so commit messages must reference the epic, not a phantom
	# issue.
	local epic_id
	epic_id=$(epic_id_from_slug "$feature_slug")

	local issues_ledger="$repo_root/${specs_directory}/issues.jsonl"

	# Build and persist the contract
	local contract_json
	contract_json=$(jq -n \
		--arg status "READY" \
		--arg phase "research" \
		...
		--arg explore_md_path "$explore_md_abs" \
		--arg explore_md_rel "${feature_dir_rel}/explore.md" \
		--arg design_target "$design_target_rel" \
		--arg design_target_abs "$design_target_abs" \
		--arg data_model_target "$data_model_target_rel" \
		--arg data_model_target_abs "$data_model_target_abs" \
		...
		'{
            ...
            data_model_target: $data_model_target,
	...
	# Persist for the post-script
	local contract_path="${DEVIATE_RESEARCH_CONTRACT_PATH:-}"
	if [ -z "$contract_path" ]; then
		contract_path=$(mktemp "$(create_temp_dir "deviate-research-phase")/contract-XXXXXX.json")
	fi
	printf '%s\n' "$contract_json" >"$contract_path"

	log_ok "Upstream gate passed: $explore_md_abs"
	log_ok "Design target:     $design_target_rel"
	log_ok "Data-model target: $data_model_target_rel"

	printf '%s\n' "$contract_json"
	log_info "Contract written to: $contract_path"
	log_info "Run 'DEVIATE_RESEARCH_CONTRACT_PATH=$contract_path $SCRIPT_NAME post' after writing both artifacts."
}
...
cmd_post() {
	...
	if [ -z "$contract_path" ] || [ ! -f "$contract_path" ]; then
		...
		contract_path=$(ls -t "$temp_base"/deviate-research-phase.*/contract-*.json \
			"$HOME/.tmp"/deviate-research-phase.*/contract-*.json \
			/tmp/deviate-research-phase.*/contract-*.json 2>/dev/null | head -1 || true)
	fi

	if [ -z "$contract_path" ] || [ ! -f "$contract_path" ]; then
		log_err "Research contract not found."
		log_err "Run: $SCRIPT_NAME pre [--feature <dir>] first."
		exit 5
	fi
	...
	local design_abs
	design_abs=$(printf '%s' "$contract" | jq -r '.design_target_abs // empty')
	local data_model_abs
	data_model_abs=$(printf '%s' "$contract" | jq -r '.data_model_target_abs // empty')
	local repo_root
	...
	[ -z "$epic_id" ] && epic_id="research"

	if [ -z "$design_abs" ] || [ ! -f "$design_abs" ]; then
		log_err "design.md not found at $design_abs"
		exit 5
	fi
	if [ -z "$data_model_abs" ] || [ ! -f "$data_model_abs" ]; then
		log_err "data-model.md not found at $data_model_abs"
		exit 5
	fi

	cd "$repo_root"

	# Content validation — required headers per artifact (v2.0.0 schema)
	local design_missing=()
	local design_required=(
	...
	local header
	for header in "${design_required[@]}"; do
		if ! grep -Eq "^${header}$" "$design_abs"; then
			design_missing+=("$header")
		fi
	done

	if [ "${#design_missing[@]}" -gt 0 ]; then
		log_err "design.md content validation failed — missing required headers:"
		for h in "${design_missing[@]}"; do
			log_err "  - $h"
		done
		exit 5
	fi
	log_ok "design.md content validation passed"
	...
	for header in "${dm_required[@]}"; do
		if ! grep -Eq "^${header}$" "$data_model_abs"; then
			dm_missing+=("$header")
		fi
	done

	if [ "${#dm_missing[@]}" -gt 0 ]; then
		log_err "data-model.md content validation failed — missing required headers:"
		for h in "${dm_missing[@]}"; do
			log_err "  - $h"
		done
		exit 5
	fi
	log_ok "data-model.md content validation passed"

	# Constitutional alignment marker scan — the v2.0.0 schema stores the
	# `Alignment` value as a CELL in the CONSTITUTIONAL_ALIGNMENT_AUDIT table
	# (one of: Aligned | Tension | Violation), not as a key-value line.
	#
	# The post-script is MECHANICAL and BLIND to constitutional violations:
	# it does not refuse to commit on a Violation row. The agent-level
	# violation gate (see deviate-research.tmpl invariant #8) is the
	# sole gatekeeper. This scan emits a warning for human visibility only.
	if ! grep -Eq '\| *(Aligned|Tension|Violation) *\|' "$design_abs"; then
		log_warn "design.md has no CONSTITUTIONAL_ALIGNMENT_AUDIT rows with a valid Alignment cell — review manually before PRD"
	fi
	if grep -Eq '\| *Violation *\|' "$design_abs"; then
		log_warn "design.md contains a CONSTITUTIONAL_ALIGNMENT_AUDIT row with Alignment: Violation"
		log_warn "The post-script is mechanical and cannot refuse to commit. If the"
		log_warn "agent did not detect and halt on this violation, the human operator"
		log_warn "must review and revert the commit."
	fi

	# Atomic commit — stage and commit both files in a single commit.
	# Reference the EPIC (the feature bucket number), not a phantom issue —
	# at this phase no issue exists yet.
	local design_rel="${design_abs#${repo_root}/}"
	local dm_rel="${data_model_abs#${repo_root}/}"
	local commit_msg="docs(${epic_id}): design and data model"
	git add "$design_rel" "$dm_rel" >/dev/null 2>&1 || {
		log_err "Failed to stage $design_rel and $dm_rel"
		exit 6
	}
	if git commit --no-verify -m "$commit_msg" >/dev/null 2>&1; then
		log_ok "Committed: $commit_msg"
	else
		log_err "Commit failed for design + data model"
		exit 6
	fi

	jq -n \
		--arg status "SUCCESS" \
		--arg design "$design_rel" \
		--arg data_model "$dm_rel" \
		--arg feature_dir "$feature_dir" \
		--arg epic_id "$epic_id" \
		--arg next_action "Run the prd skill (after Gate 1: Design Approval)" \
		'{
            status: $status,
            design_target: $design,
            data_model_target: $data_model,
            feature_dir: $feature_dir,
            epic_id: $epic_id,
...
Subcommands:
  pre [<epic> | --feature <value>]   Validate explore.md, emit a JSON contract.
                                     The epic argument is an epic id (e.g. "001"),
                                     an NNN-slug, a directory path, or a path to
                                     an explore.md file. Defaults to the active
                                     marker or the most recently modified bucket.
  post                               Validate design.md and data-model.md, commit them.

Environment:
  DEVIATE_RESEARCH_CONTRACT_PATH          Override the temp file used to hand the contract from pre to post.
  RESEARCH_FEATURE_DIR            Default feature directory used by pre when no argument is given.
EOF
	exit 1
fi
...
-h | --help | help)
	cat <<EOF
$SCRIPT_NAME - Orchestrator for the /deviate-research macro layer (PHASE_RESEARCH)
