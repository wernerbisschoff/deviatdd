...
#
# deviate-red.sh - Orchestrator for /deviate-red (TDD RED phase)
#
# This script wraps the operational concerns of the TDD RED phase:
...
#   - Stage test files and commit
#
# Usage:
#   deviate-red.sh pre     # Discover task, emit JSON contract
#   deviate-red.sh post    # Stage test files and commit
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for /deviate-red (TDD RED phase)
...
SUBCOMMANDS
	...
	post    Stage test files and commit
...
EXAMPLES
	...
	$SCRIPT_NAME post    # Stage and commit test files
EOF
}
...
locate_active_task() {
	...
	# Find the most recent tasks.md with pending or in-progress tasks
	local candidates
	candidates=$(find "$REPO_ROOT/specs" -name "tasks.md" -type f 2>/dev/null || true)

	if [[ -z "$candidates" ]]; then
		# Fallback: check repo root for tasks.md
	...
	# For each candidate, find the first pending or in-progress task
	while IFS= read -r candidate; do
		...
		local pending_line
		pending_line=$(grep -nE '^\s*-\s+\[ \]\s+\[T[0-9]+\]' "$candidate" | head -1 || true)
		...
		if [[ -n "$pending_line" ]]; then
			...
			# Extract task ID (e.g., T001, TASK-104)
			task_id=$(echo "$pending_line" | grep -oE '\[T[0-9]+\]' | head -1 | tr -d '[]' || echo "")
			if [[ -z "$task_id" ]]; then
				task_id=$(echo "$pending_line" | grep -oE 'TASK-[0-9]+' | head -1 || echo "T001")
			fi

			# Extract task title (everything after [T001])
			local line_num
...
extract_constitution_commands() {
	...
	# Fallback to code block fences: look for ```bash blocks with test/lint entries
	if [[ -z "$test_command" ]]; then
		test_command=$(grep -A2 'test_command' "$constitution_file" 2>/dev/null |
			tail -1 | sed -E 's/^[[:space:]]*//' || true)
	fi
	if [[ -z "$lint_command" ]]; then
		lint_command=$(grep -A2 'lint_command' "$constitution_file" 2>/dev/null |
			tail -1 | sed -E 's/^[[:space:]]*//' || true)
	fi
...
# ── Extract feature slug from spec_dir ──────────────────────────────────
extract_feature_slug() {
	local slug=""
	if [[ -n "$SPEC_DIR" ]]; then
		slug=$(basename "$SPEC_DIR" 2>/dev/null || echo "")
	fi
	if [[ -z "$slug" ]]; then
		slug="unknown"
	fi
...
# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-red pre-phase starting..."
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-red post-phase starting..."
	...
	# Stage all tracked changes (test files written by LLM)
	git add -u
	...
	local commit_subject="test($TASK_ID): add failing acceptance criteria tests"
	if ! git commit -m "$commit_subject" -m "Phase: RED"; then
