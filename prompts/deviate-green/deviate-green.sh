...
#
# deviate-green.sh - Orchestrator for /deviate-green (TDD GREEN phase)
#
# This script wraps the operational concerns of the TDD GREEN phase:
...
#
# Usage:
#   deviate-green.sh pre    # Discover task, emit JSON contract
#   deviate-green.sh post   # Stage implementation files and commit
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for /deviate-green (TDD GREEN phase)
...
SUBCOMMANDS
	...
	post    Stage implementation files and commit
...
EXAMPLES
	...
	$SCRIPT_NAME post    # Stage and commit implementation files
EOF
}
...
locate_active_task() {
	...
	while IFS= read -r candidate; do
		...
		# First look for in-progress tasks (RED phase just completed)
		local active_line
...
# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-green pre-phase starting..."
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-green post-phase starting..."
	...
	# Stage all tracked changes (implementation files written by LLM)
	git add -u
	...
	local commit_subject="feat($TASK_ID): implement minimal logic to pass acceptance tests"
	if ! git commit -m "$commit_subject" -m "Phase: GREEN"; then
