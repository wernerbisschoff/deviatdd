...
#
# deviate-e2e.sh - Orchestrator for /deviate-e2e (TDD E2E phase)
#
# This script wraps the operational concerns of the TDD E2E phase:
#   - Locate the project spec context
#   - Detect project type (CLI/Web/API/Library)
#   - Check all phases are complete
#   - Resolve E2E test configuration
#   - Emit JSON contract with project context
#   - Stage E2E test files and commit
#
# Usage:
#   deviate-e2e.sh pre    # Discover context, detect project type, emit JSON contract
#   deviate-e2e.sh post   # Stage E2E test files and commit
#
# Exit codes:
#   0   Success
#   1   Invalid arguments
...
#   4   Phase completion check failed
#
set -euo pipefail
...
show_help() {
	cat <<EOF
$SCRIPT_NAME - Orchestrator for /deviate-e2e (TDD E2E phase)

SYNOPSIS
	$SCRIPT_NAME <pre|post>

SUBCOMMANDS
	pre     Discover context, detect project type, check phases, emit JSON contract
	post    Stage E2E test files and commit
...
EXAMPLES
	$SCRIPT_NAME pre     # Discover context and emit contract
	$SCRIPT_NAME post    # Stage and commit E2E test files
EOF
}
...
find_spec_dir() {
	...
	# Look for specs/ directories with tasks.md
	local candidates
	...
	if [[ -z "$candidates" ]]; then
		...
		spec_dir=$(dirname "$first")
		spec_dir=$(realpath --relative-to="$REPO_ROOT" "$spec_dir" 2>/dev/null || echo "$spec_dir")
	fi
...
detect_project_type() {
	...
	if [[ -f "$REPO_ROOT/package.json" ]]; then
		...
		if grep -qE '"bin"|"cli"' "$REPO_ROOT/package.json" 2>/dev/null; then
			project_type="CLI"
			e2e_strategy="CLI"
		# Check for frontend framework
		elif grep -qE '"react"|"vue"|"next"|"playwright"|"cypress"' "$REPO_ROOT/package.json" 2>/dev/null; then
			project_type="Web"
			e2e_strategy="Web"
		else
		...
		# Phoenix/Elixir project (API or Web)
		if ls "$REPO_ROOT"/lib/*_web* >/dev/null 2>&1; then
			project_type="Web"
			e2e_strategy="Web"
		else
			project_type="API"
		...
		# Python project
		if grep -qE 'entry_points|console_scripts' "$REPO_ROOT/pyproject.toml" 2>/dev/null ||
			grep -qE 'entry_points|console_scripts' "$REPO_ROOT/setup.py" 2>/dev/null; then
			project_type="CLI"
			e2e_strategy="CLI"
		elif [[ -d "$REPO_ROOT/tests/e2e" ]]; then
			project_type="API"
			e2e_strategy="API"
		else
			project_type="Library"
		...
		if grep -qE '\[\[bin\]\]' "$REPO_ROOT/Cargo.toml" 2>/dev/null; then
			project_type="CLI"
			e2e_strategy="CLI"
		else
			project_type="Library"
...
# ── Resolve E2E test command ────────────────────────────────────────────
resolve_e2e_command() {
	local e2e_command=""

	case "$E2E_STRATEGY" in
	CLI)
...
# ── Extract test_command from constitution ──────────────────────────────
...
check_phases_complete() {
	...
	if [[ ! -f "$tasks_file" ]]; then
		log_info "No tasks.md found — phase completion cannot be verified"
		echo '{"status":"PHASES_INCOMPLETE","reason":"NO_TASKS_FILE","repo_root":"'"$REPO_ROOT"'"}'
		exit 4
	fi

	# Read tasks.md and check phases
	local current_phase=""
	local phase_has_incomplete=false

	while IFS= read -r line; do
		# Track phase headers
		if echo "$line" | grep -qE '^## '; then
			# Check previous phase before switching
			if [[ -n "$current_phase" ]] && [[ "$phase_has_incomplete" == "true" ]]; then
				all_complete=false
				if [[ -z "$incomplete_phases" ]]; then
					incomplete_phases="$current_phase"
				else
					incomplete_phases="$incomplete_phases, $current_phase"
				fi
			fi
			current_phase=$(echo "$line" | sed -E 's/^##+\s+//')
			phase_has_incomplete=false
		fi
		...
		if echo "$line" | grep -qE '^\s*-\s+\[[ /x]\]\s+\[T[0-9]+\]'; then
			...
			if echo "$line" | grep -qE '^\s*-\s+\[[ /]\]'; then
				# In-progress or pending — incomplete
				phase_has_incomplete=true
	...
	PHASES_ALL_COMPLETE="$all_complete"
	INCOMPLETE_PHASES="$incomplete_phases"
}

# ── Pre subcommand ───────────────────────────────────────────────────────
cmd_pre() {
	log_info "Deviate-e2e pre-phase starting..."

	validate_repo
	log_ok "Repository validated: $REPO_ROOT"

	find_spec_dir
	log_ok "Spec directory: ${SPEC_DIR:-<none>}"

	detect_project_type
	log_ok "Project type: $PROJECT_TYPE"
	log_ok "E2E strategy: $E2E_STRATEGY"

	resolve_e2e_command
	log_ok "E2E command: ${E2E_COMMAND:-<none>}"
	...
	if [[ "$PHASES_ALL_COMPLETE" == "false" ]]; then
		log_warn "Not all phases complete. Incomplete: $INCOMPLETE_PHASES"
		emit_json_contract \
			"status" "PHASES_INCOMPLETE" \
			"phase" "e2e" \
			"spec_dir" "$SPEC_DIR" \
			"project_type" "$PROJECT_TYPE" \
			"e2e_strategy" "$E2E_STRATEGY" \
		...
		log_ok "Pre-phase complete (incomplete phases). Contract emitted to stdout."
		exit 0
	fi
...
# ── Post subcommand ──────────────────────────────────────────────────────
cmd_post() {
	log_info "Deviate-e2e post-phase starting..."
	...
	# Stage all tracked changes (E2E test files written by LLM)
	git add -u
	...
	# Stage E2E test directories
	if [[ -d "$REPO_ROOT/tests/e2e" ]]; then
		git add "$REPO_ROOT/tests/e2e/" 2>/dev/null || true
	fi
	...
	local commit_subject="test(e2e): add end-to-end verification tests"
	if ! git commit -m "$commit_subject" -m "Phase: E2E"; then
