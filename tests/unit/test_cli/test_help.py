...
def test_help_user_panel_has_exactly_four_commands():
    ...
    ``meso``, ``micro`` as command rows. If a phase dispatcher or
    agent-internal command ever leaks into this panel, a first-timer
    ...
    assert user_command_names == {
        ...
        f"User panel must contain exactly {{setup, run, meso, micro}} "
