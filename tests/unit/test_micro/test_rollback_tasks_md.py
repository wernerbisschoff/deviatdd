...
class TestRollbackPreservesTasksMd:
    ...
    def test_execute_rollback_checkouts_tasks_md_dropped_by_boundary(
        ...
        head = _sha(root)
        # Reset still targets the caller boundary. ISS-ADH-040 may then
        # commit ``chore(...): restore tasks.md after JUDGE rollback``
        # on top so ``git ls-tree HEAD`` lists the queue.
        parent = (
            subprocess.run(
                ["git", "rev-parse", "HEAD^"],
                cwd=root,
        ...
        assert plan_sha in {head, parent}, (
