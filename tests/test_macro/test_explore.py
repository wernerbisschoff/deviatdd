...
class TestExploreCommand:
    ...
    def test_explore_transitions_from_idle(self, tmp_path: Path):
        with chdir(tmp_path):
    ...
    def test_explore_rejects_if_not_idle(self, tmp_path: Path):
        with chdir(tmp_path):
            dot_dir = Path(".deviate")
            dot_dir.mkdir(parents=True)
    ...
    def test_explore_missing_session_file_defaults_idle(self, tmp_path: Path):
        with chdir(tmp_path):
            dot_dir = Path(".deviate")
            dot_dir.mkdir(parents=True)
    ...
    def test_explore_missing_dotdeviate_dir_error(self, tmp_path: Path):
        with chdir(tmp_path):
