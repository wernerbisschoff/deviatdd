...
class TestGreenPostYellowHandoff:
    def test_green_post_tamper_detected_transitions_to_yellow(self, tmp_git_repo: Path):
        with chdir(tmp_git_repo):
            dot_dir = Path(".deviate")
            dot_dir.mkdir(parents=True)
            session = SessionState(current_phase="RED", active_issue_id="ISS-002-005")
            session.save(dot_dir / "session.json")

            task = _make_task_record(
                task_id="TSK-005-04",
                issue_id="ISS-002-005",
                description="YELLOW handoff test — tamper detected",
                status="RED",
            )
            ledger_path = Path("specs") / "005-micro-layer" / "tasks.jsonl"
            _write_ledger(ledger_path, task)

            test_file = Path("tests") / "test_handoff.py"
            test_file.parent.mkdir(parents=True)
            test_file.write_text("def test_pass():\n    assert True\n")
            ...
            assert session_data.get("current_phase") == "YELLOW", (
                f"Expected session to be YELLOW, got {session_data.get('current_phase')}"
            )
            ...
            assert "YELLOW" in statuses, (
                f"Expected YELLOW in ledger statuses: {statuses}"
            )

    def test_green_post_tamper_pass_stays_green(self, tmp_git_repo: Path):
        with chdir(tmp_git_repo):
            dot_dir = Path(".deviate")
            dot_dir.mkdir(parents=True)
            session = SessionState(current_phase="RED", active_issue_id="ISS-002-005")
            ...
            task = _make_task_record(
                task_id="TSK-005-04",
                issue_id="ISS-002-005",
                description="YELLOW handoff test — no tamper",
                status="RED",
            )
            ledger_path = Path("specs") / "005-micro-layer" / "tasks.jsonl"
            _write_ledger(ledger_path, task)

            test_file = Path("tests") / "test_no_tamper.py"
            test_file.parent.mkdir(parents=True)
            test_file.write_text("def test_pass():\n    assert True\n")
            ...
            assert session_data.get("current_phase") == "GREEN", (
                f"Expected session to be GREEN, got {session_data.get('current_phase')}"
            )

            ledger_lines = ledger_path.read_text(encoding="utf-8").strip().split("\n")
            statuses = [json.loads(line).get("status") for line in ledger_lines if line]
            assert "YELLOW" not in statuses, (
                f"Did not expect YELLOW in ledger: {statuses}"
            )

    def test_green_post_yellow_triggered_by_tamper_not_commit(self, tmp_git_repo: Path):
        with chdir(tmp_git_repo):
            dot_dir = Path(".deviate")
            dot_dir.mkdir(parents=True)
            session = SessionState(current_phase="RED", active_issue_id="ISS-002-005")
            ...
            task = _make_task_record(
                task_id="TSK-005-04",
                issue_id="ISS-002-005",
                description="YELLOW handoff test — trigger by tamper not commit",
                status="RED",
            )
            ledger_path = Path("specs") / "005-micro-layer" / "tasks.jsonl"
            _write_ledger(ledger_path, task)

            test_file = Path("tests") / "test_tamper_commit.py"
            test_file.parent.mkdir(parents=True)
            test_file.write_text("def test_pass():\n    assert True\n")
            ...
            assert "TAMPER_DETECTED" in result.output, (
                f"Expected TAMPER_DETECTED: {result.output}"
            )
            ...
            assert session_data.get("current_phase") == "YELLOW", (
                f"Expected YELLOW session, got {session_data.get('current_phase')}"
            )
