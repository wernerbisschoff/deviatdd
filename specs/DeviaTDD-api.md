...
* **Aligned validator contract:** prompt schema lists agree one-to-one with `ARTIFACT_VALIDATORS` required lists per phase (`src/deviate/core/validation.py`). `Session State` is not a PRD contract section (it rides manifest JSON); `Source Registry` is not required (folded into Document Control). Empty mandated sections fail substance checks naming the section; File Registry (12 rows) and Risk Register (4 rows) warn at row-count caps without failing; PRD FR sub-fields Preconditions, State Transition, Exception are omit-if-N/A (absent passes, present-but-empty fails). Shipped pre-alignment sample artifacts under `specs/` are grandfathered and exempt from revalidation; unmarked legacy failures still fail.
