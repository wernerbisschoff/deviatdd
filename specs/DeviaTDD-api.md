...
* **Description:** Discovers epic, validates `prd.md` exists, computes the next `ISS-{NNN}`
  ...
  `next_issue_id`.
* **Common Flags:** `--json`, `--quiet`
* **Codebase Structure Appendix (ISS-ADH-008):** The contract dict is enriched with a
  `codebase_structure_appendix` field generated by `_build_codebase_structure_appendix()`
  (in `macro.py`). This function scans `src/` for all Python files, calls
  `extract_file_structure()` from `deviate.core.treesitter` on each file, and formats
  the result as a `## Codebase Structure` appendix. The shard agent prompt template
  (`src/deviate/prompts/auto/shard.md`) consumes this appendix via the
  `${codebase_structure_appendix}` placeholder between the PRD reading and vertical
  slicing steps.
...
* **Common Flags:** `--json`, `--quiet`
* **Codebase Structure Artifact (ISS-ADH-008):** After the complexity gate but before
  emitting the contract, `adhoc pre` runs `_build_codebase_structure_artifact()` (in
  `adhoc.py`). This function scans the directory specified by `--scan-dir` (default
  `src/`) for Python files, calls `extract_file_structure()` from
  `deviate.core.treesitter` on each file, and writes a `## Codebase Structure` markdown
  artifact to `specs/adhoc/codebase_structure.md`. The artifact path is emitted in the
  contract as `codebase_structure_path`. The `/deviate-adhoc` skill prompt
  (`src/deviate/prompts/skills/deviate-adhoc/SKILL.md`) step 3 instructs the agent to
  read this artifact first for pre-extracted file signatures before falling back to
  grep/glob.
...
* **Common Flags:** `--json`, `--quiet`
* **File Structure Appendix (ISS-ADH-008):** During `_invoke_agent_phase("plan", ...)`,
  the prompt is enriched with a `## Target File Structure` appendix. The function
  `_enrich_plan_prompt()` (in `meso.py`) reads the issue spec's `## System Topology
  Mapping` section, finds `Primary Architectural Workstations` file paths, and calls
  `extract_file_structure()` from `deviate.core.treesitter` on each existing Python
  file. The result (imports, function signatures, class methods) is injected into the
  contract as `file_structure_appendix` before the agent prompt is assembled. Missing
  files are skipped with a warning; absent `System Topology Mapping` section produces
  an empty appendix.
...
* **Structured Diff Injection (ISS-ADH-008):** After collecting raw `git diff RED..HEAD`
  output, calls `extract_changed_symbols()` from `deviate.core.treesitter` to produce a
  `## Structured Diff Summary` listing only changed `FunctionDef`/`ClassDef` nodes with
  old/new signatures. This summary is prepended to the agent prompt, reducing V4 Pro
  token consumption on diff parsing by ~15x (raw diff appended below for full context).
  When the diff is empty or contains no parseable symbols, no structured section is
...
* **Description:** Verifies a GREEN transition exists. Appends REFACTOR transition, runs
  tree-sitter-based structural checks, runs pytest before/after to detect regression.
  On regression, restores via `git restore .` and halts. Commits with
  `refactor({scope}): REFACTOR phase - code cleanup`.
* **Extended Structural Checks (ISS-ADH-008):** The legacy `_check_return_type_mismatch`
  (stdlib `ast.parse`) was replaced with tree-sitter incremental parsing. Checks now
  include:
  1. **Return type mismatch** (preserved): validates `str`/`int`/`float`/`bool`/`list`/
     `dict`/`tuple`/`set` annotated returns match actual `return` value types.
  2. **Dead code detection:** private functions (prefixed `_`) with zero call sites are
     flagged as `DEAD_CODE:` issues.
  3. **Duplicated code blocks:** Similar AST subtrees ≥5 lines across functions are
     flagged as `DUPLICATED_CODE:` issues.
  4. **Cyclomatic complexity warnings:** Functions with complexity ≥10 (counting
     `if`/`elif`/`for`/`while`/`except`/`with`/boolean_operator nodes) are flagged as
     `COMPLEXITY:` issues.
  Uses `incremental_parse()` so large file re-scans re-parse only changed ranges.
  Replaced stdlib `ast` import entirely — all checks run through tree-sitter queries.
...
│   ├── treesitter.py         # Tree-sitter parser factory, symbol extraction, incremental parse
...
### 1.5 Tree-Sitter Structural Analysis Module (ISS-ADH-008)

The `src/deviate/core/treesitter.py` module provides Python AST parsing via tree-sitter,
enabling structural code analysis across JUDGE, PLAN, REFACTOR, SHARD, and ADHOC phases.
It uses `tree-sitter-python` (bundled grammar) for deterministic, incremental parsing.

| Public Function | Signature | Description |
|---|---|---|
| `get_parser()` | `() -> Parser` | Returns a cached singleton `tree_sitter.Parser` |
| `extract_changed_symbols()` | `(diff_text: str) -> list[dict]` | Parses `git diff` hunks, extracts changed `FunctionDef`/`ClassDef` with old/new signatures and file paths. Returns empty list for empty or unparseable diffs. |
| `extract_file_structure()` | `(source: str) -> dict` | Parses a source string and returns `{functions: {...}, classes: {...}, imports: [...]}` with extracted parameters, return types, and method lists. |
| `incremental_parse()` | `(source: str, old_tree: Tree \| None) -> Tree \| None` | Parses source using an optional existing `old_tree` — tree-sitter re-parses only changed byte ranges. Used for efficient re-scans of large modified files. |

**Integration points across phases:**

| Phase | Integration | Function Called | Output |
|---|---|---|---|
| JUDGE | `_run_judge_phase()` appends `## Structured Diff Summary` | `extract_changed_symbols()` | ≤500 token structured diff (vs raw diff up to 3000+) |
| PLAN | `_enrich_plan_prompt()` injects `## Target File Structure` | `extract_file_structure()` | Function/class signatures from `Primary Architectural Workstations` files |
| REFACTOR | `_ts_check_return_type()`, `_ts_check_dead_code()`, `_ts_check_cyclomatic_complexity()` | `incremental_parse()`, `get_parser()` | Return type mismatch, dead code, duplicated code, complexity warnings |
| SHARD | `_build_codebase_structure_appendix()` in `shard_pre()` | `extract_file_structure()` | `## Codebase Structure` appendix in contract |
| ADHOC | `_build_codebase_structure_artifact()` in `adhoc pre` | `extract_file_structure()` | `specs/adhoc/codebase_structure.md` artifact |

**Dependencies:** `tree-sitter>=0.24`, `tree-sitter-python` (bundled, no runtime grammar
compilation required). Module-level `_get_parser()` caches the singleton parser instance.

**Performance budgets:** `extract_file_structure()` ≤200ms (initial parse), ≤20ms
(incremental); `extract_changed_symbols()` ≤100ms; `_check_return_type_mismatch()` (upgraded)
≤300ms (initial), ≤50ms (incremental).

**Defensive exclusions:** RED, GREEN, YELLOW, TASKS, and MACRO phases (explore/research/prd)
do NOT invoke tree-sitter. The stdlib `ast` import has been removed from `micro.py` — all
structural checks now use tree-sitter exclusively.
