...
`timeout_seconds`, `claim_remote`, `[agent].backend`). Inline comments sit on the same line as
...
* **`.git/info/exclude` exclusions:** `_ensure_root_gitignore` adds
  ...
  directories deep).
* **Tests:** `TestInstallDeviatddSkill` in `tests/unit/test_cli/test_init.py` and
...
* **Aligned validator contract:** prompt schema lists agree one-to-one with `ARTIFACT_VALIDATORS` required lists per phase (`src/deviate/core/validation.py`). `Session State` is not a PRD contract section (it rides manifest JSON); `Source Registry` is not required (folded into Document Control). Empty mandated sections fail substance checks naming the section; File Registry (12 rows) and Risk Register (4 rows) warn at row-count caps without failing; PRD FR sub-fields Preconditions, State Transition, Exception are omit-if-N/A (absent passes, present-but-empty fails). A non-empty data-model `## Data Flow` must contain a fenced `sequenceDiagram` or `None — local-only`, and an Outbound integrations heading/table or the same local-only marker (`validate_artifact` / `research post`); the heading name stays `Data Flow` (request flow / runtime scenarios, not schema prose). Shipped pre-alignment sample artifacts under `specs/` are grandfathered and exempt from revalidation; unmarked legacy failures still fail.
...
* **Source:** `src/deviate/cli/meso.py`
* **Description:** Claim an issue and create its worktree. With an explicit `<issue-id>` (positional, or `--issue` when no positional is given),
...
* **Acceptance ownership:** The issue supplies stories, scope, topology, `AO-NNN` outlines, edge cases, and performance constraints. Plan reconciles each outline into complete `AC-PLAN-NNN` scenarios with Source Outline, upstream traceability, current-code evidence, and Given/When/Then. This contract is authoritative for Tasks, RED, and JUDGE.
  ...
  * **Forbidden patterns** (any one triggers `PLAN_ACCEPTANCE_CONTRACT_INVALID` from `deviate plan post`): Source Outline labelled `Edge Cases`, `Boundary`, `Constitutional §…`, `RLS`, `Tenant Isolation`, `Hardening`, `Security`, or any non-AO string; missing `**Source Outline**` / `**Upstream Traceability**` / `**Current-Code Evidence**` / any of `**Given**` / `**When**` / `**Then**`; a repeated or illegal `**Verification Mode**: <automated|manual|deferred>` literal (a scenario MUST carry exactly one legal mode line; an empty or non-alphabetic value is treated as missing); an issue AO not used by any AC-PLAN scenario; duplicate or non-sequential `AC-PLAN-NNN` identifiers; wrapping the plan body in any XML tag / code fence / preamble. A missing mode line is a loud error citing the scenario id: the gates fail with `PLAN_ACCEPTANCE_CONTRACT_INVALID` and no default is inserted. The validator lives at `src/deviate/core/validation.py::validate_acceptance_contract`; `repair_missing_verification_mode` never inserts a default.
  * **Traceability gate:** before emitting the contract, `_plan_pre` runs `validate_issue_traceability` (`src/deviate/core/validation.py`) over the issue body. Status values are `READY`, `NOT_READY`, and `ISSUE_NOT_FOUND`. `NOT_READY` names each missing field in `missing_fields` plus a `repair_hint`. The gate checks section names only, not prose: `User Stories Ledger`, `Upstream Requirement Tracing`, `Acceptance Outline`, plus at least one `AO-NNN` token in the outline. Legacy issues regain the missing sections via `repair_issue_traceability` (`src/deviate/cli/meso.py`), which inserts only absent sections under `specs/` and revalidates. Shard (`src/deviate/prompts/auto/shard.md`) and adhoc (`src/deviate/prompts/commands/deviate-adhoc.md`) emit these section names and token families so new issues pass without repair.
  * **Soft handover checklist:** on an interactive TTY (and not `--json` / `--quiet`), contract-mode `_plan_pre` prints a 12-point “handover ready?” checklist from `src/deviate/prompts/handover.py` so a reviewer can confirm the epic+issue pack is understandable. The print never prompts, never waits, and is not Gate 2. Off-TTY / `--json` / `--quiet` / `deviate meso run` (`_silence_stdout`) skip the banner or emit one log line. The same `format_handover_checklist()` text is injected into the auto Plan prompt (`load_template("plan")`) and the installed `/deviate-plan` overlay so manual and auto see the same contract when human-facing.
...
Validates plan.md exists, is non-empty, and contains a valid Acceptance Contract; auto-renders HTML when changed, commits with convention-aware messaging, and transitions to TASKS. Missing/malformed contracts fail as `PLAN_ACCEPTANCE_CONTRACT_MISSING` / invalid contract diagnostics. A scenario missing its `**Verification Mode**:` line fails loudly with `PLAN_ACCEPTANCE_CONTRACT_INVALID` citing the scenario id; no default is inserted. An existing invalid or duplicated mode literal blocks the same way.
...
* **Validation:** reports PLAN_NOT_FOUND, PLAN_ACCEPTANCE_CONTRACT_MISSING, or PLAN_ACCEPTANCE_CONTRACT_INVALID; no Gherkin fallback. A contract missing a `**Verification Mode**:` line fails loudly with `PLAN_ACCEPTANCE_CONTRACT_INVALID` citing the scenario id; no default is inserted.
...
* **Description:** Runs the project's resolved test command (language-agnostic: `mix test`,
  ...
  Commands already starting with `mise run` or `mise exec` remain unchanged; the runner does not nest mise commands.
...
* **Single-Task (`deviate micro run <task-id>`):** Triggers the automated
  ...
  * **Green → Judge → Green loop (TDD only):** `_run_tdd_cycle` wraps the
    ...
    fixtures/tests about the same shared identity token), flips unit↔
    integration layer, or oscillates A-B-A with conflicting requested vs
    provider values for the same returned identity, the runner logs
    `JUDGE_REQUIREMENT_CONTRADICTION`, marks `HITL_PENDING`,
    raises `HitlEscalationError`, and does not consume further GREEN/RED train
    budget (GH-230, GH-253). Unit↔integration layer oscillation on a stamped
    task is prevented earlier by `<layer_lock>` (GH-248); this HITL remains
    the backstop. A-B-A token similarity alone is not a contradiction:
    additive restatements (changelog + verification, coverage + evidence)
    still train. Generic `matching` wording without an identity, explicit
    ...
    missing at the new HEAD (`git checkout <pre_reset_sha> -- <path>`;
    GH-201 — do not `PhaseFailedError` solely because pre-RED was
    computed too early). TDD JUDGE's `revert_green`
    passes the remapped `session.red_commit_sha` (subject-match after a
    commit-train rebase; GH-168), TDD JUDGE's `revert_red` resolves
    `red_commit_sha^` via `_resolve_revert_red_boundary` (remap a
    ...
    boundary or the boundary is empty/whitespace. A `boundary_sha` that
    is not an ancestor of HEAD raises `ROLLBACK_STALE_BOUNDARY` and
    leaves the active branch unchanged. `revert_green` with a stored RED
    SHA that is not an ancestor and cannot be remapped raises
    `ROLLBACK_STALE_RED_SHA` and does not reset. The rejected-commit
    ...
    failures therefore leave no cross-task rollback anchor. Resume that
    skips RED as already done also rebinds `session.red_commit_sha` to
    this task (`_bind_session_red_boundary`): a leftover SHA whose
    subject names a completed predecessor is cleared and recovered from
    this task's ledger plus git. JUDGE `revert_green` / `revert_red`
    refuse that predecessor SHA so rollback cannot reset through a
    completed earlier task (GH-247). Existing-test / passing RED appends
    a RED ledger row before GREEN (`_ensure_red_ledger_transition`) so
    `green post` cannot see `PENDING` (`GREEN_GUARD_REJECTED`). TDD
    `revert_green` with an empty `session.red_commit_sha` raises
    `PhaseFailedError("DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING ...")`
    including planned `head_sha` and `recovery_ref`. `_run_judge_phase`
  ...
  * **EXECUTE commit-failure recovery (terminal contract):** The single
  ...
  Auto RED and GREEN receive one `<train_feedback>` section containing every complete JUDGE round in recorded order.
  Prompt insertion escapes `&`, `<`, and `>` as XML character references. Stored feedback remains unchanged.
  ...
  Existing malformed cards require explicit repair; this change does not infer ownership of previously orphaned feedback lines.
  ...
  * **JUDGE `next_action` routing:** After the TDD mechanical evidence gate accepts a forward PASS, the runner honors `HandoverManifest.next_action`
    ...
    **JUDGE `next_action` Routing Table** in this document for rollback anchors
    ...
    note. `_rewrite_unmatched_tdd_pass` returns the forward action
  ...
  * **Verification tasks:** `Verification_Batch` uses CHECKPOINT, not RED/GREEN.
    ...
    Dispatch raises `PhaseFailedError` for these responses; the runner must not report successful completion (#265).
...
* **GREEN Stub-PASS Guard (REMOVED):** An earlier revision of this spec
  ...
  cross-check only.
* **JUDGE Phase Attribution:** Auto JUDGE receives `<green_diff>` from the current GREEN-entry boundary through HEAD, including dirty and untracked changes. The combined `<diff>` still supplies RED and GREEN evidence. Attribute GREEN scope violations only to `<green_diff>`. Route RED-only scope violations to `revert_red`. Runner-authored ledger transitions and JUDGE feedback are not agent scope violations.
* **JUDGE Failed-GREEN Worktree Visibility:** When GREEN leaves production changes uncommitted because its test command failed, JUDGE evaluates both the committed RED-parent-to-HEAD diff and the current staged, unstaged, and untracked worktree diff. Untracked files are rendered with `git diff --no-index /dev/null <path>`. This preserves the implementation for compliance assessment instead of presenting JUDGE with a false RED-only view. Binary and non-UTF-8 files cannot crash this assembly: diffs are decoded with `errors='replace'`, binary paths become a one-line stub (`Binary file <path> added|modified|deleted (<n> bytes)`), and `git diff --text` / `-a` is never passed (GH-186).
* **GREEN TEST_FAILURE must not complete:** When GREEN implements (agent SUCCESS) and the suite is still red, `_run_green_phase` logs `TEST_FAILURE` and keeps the implementation for JUDGE. Detection is precise: the distinctive test-dump prefix plus empty `session.failure_kind` — not "any `train_feedback` on GREEN", which also matches mechanical / test_defect `status: FAILURE`. JUDGE may still run. Only `revert_green` (TRAIN / retry GREEN) and `revert_red` (escalate RED) change the route. `continue_refactor` / `skip_refactor` / `proceed_to_refactor_no_diff` / bare `COMPLIANCE_PASS` remap to TRAIN with the test dump. `_finish_tdd_cycle` refuses REFACTOR and COMPLETED while that TEST_FAILURE is in effect. Mechanical / test_defect GREEN `status: FAILURE` routing is unchanged. Pinned by `tests/unit/test_micro/test_orchestration.py::TestMicroOrchestration::test_green_test_failure_compliance_pass_continue_refactor_retrains`.
* **GREEN Entry Invariant:** `_run_green_phase` (`src/deviate/cli/micro.py`) invokes the GREEN agent only when `session.red_commit_sha` is a standing RED-phase failing-test boundary. Empty or whitespace SHA raises `PhaseFailedError` carrying `GREEN_ENTRY_REFUSED`. A `docs(...): add judge feedback for retry` SHA is accepted only when it rests on a RED-phase ancestor (TRAIN). A migration-bearing rollback that raises `ROLLBACK_RECOVERY_HOOK_MISSING` or `ROLLBACK_RECOVERY_HOOK_FAILED` is `ENV_NOT_READY` (GH-240): `_apply_judge_verdict` does not print `ROLLBACK_FAILED … proceeding with train feedback`, does not call `_commit_judge_feedback_and_advance`, and does not `force_transition_to("GREEN")` or enter REFACTOR on the unrestored catalog. After a non-fatal `ROLLBACK_FAILED` the runner still trains GREEN, but `_maybe_advance_red_sha_past_feedback` does not stamp `red_commit_sha` onto a feedback commit that sits on GREEN. GREEN entry recovers an unusable stored SHA from ledger + git before the agent runs (GH-236). If the boundary is still missing after recovery, a present leftover SHA raises `DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` rather than `GREEN_ENTRY_REFUSED` that prints that SHA. After JUDGE `revert_red` / cycle `no_failing_test_adjudicated` / `escalate_to_red`, `_tdd_pre_green_decision` returns `escalate` and `_run_tdd_cycle` re-dispatches RED or raises `TRAIN_EXHAUSTED` / `PhaseFailedError`. It never calls `_run_green_phase` on that path. `_consume_retry_gate_after_red` (called from `_escalate_to_new_red`) consumes `revert_red` only after `_has_red_commit_boundary` is true. `skip_refactor` / bare `COMPLIANCE_PASS` still complete without GREEN via `_NO_FAILING_TEST_FORWARD_ROUTES`. `_run_green_phase` always persists a recoverable `red_commit_sha` and then applies `_require_green_entry_red_sha` before invoking the GREEN agent, including when `current_phase` is already GREEN. TDD `revert_green` with an empty SHA raises `PhaseFailedError` carrying `DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` plus the planned `head_sha` and `recovery_ref` (never `HEAD~1`) and must not print `ROLLBACK_FAILED` and proceed. The `deviatdd` skill classifies that harness failure as `DEVIATDD_BUG` (not only `BLOCKED`) and recommends `/deviate-green` after preserving the recovery details.
...
* **Description:** Gathers the four-look map inputs for THIS issue/PR at HITL Gate 3:
  ...
  skill must emit a ≤6-line human cover sheet (intent, deviations or `None`,
  evidence/check command, Ops/ADR/Data Flow or `None`) as look 0 or a preamble
  before look 1/4, then (a) brief location + this issue's plan AC lines if present,
...
└── {FEATURE_SLUG}/           # Feature workspace bucket (e.g. ``001-...``, ``002-...``, ``adhoc``)
    ...
    ├── data-model.md         # Entity relationships, schemas, request-flow / runtime Data Flow
...
| `/deviate-research` | Architect (Expensive) | `specs/{FEATURE_SLUG}/design.md`, `data-model.md` | `deviate research pre/post` | 5 steps: read explore.md, write the floor then attack it in one agent / two ordered jobs (no sequential sub-agent spawn), produce design.md + data-model.md (schema = floor only; `## Data Flow` is request-flow / runtime scenarios with a fenced `sequenceDiagram` or `None — local-only`), commit. Existing constitutions stay read-only except greenfield bootstrap. |
...
| `/deviate-flows` | Flow Author (optional `product` pack) | `specs/_product/flows/flows-<domain>.md`, `specs/_product/flows/index.md` | (none — manual slash command only) | Conversation-first discovery, assign next `FLOW-NN`, write flow block within length budgets, append index row, single commit on sign-off. No ledger, no `flow_refs`. |
| `/deviate-architecture` | Product Architect (optional `product` pack) | `specs/_product/architecture.md`, `specs/_product/domain-model.md` | (none — manual slash command only) | Cross-epic components, contracts, data ownership, dependency graph; ADRs for hard-to-reverse tradeoffs; libref-grounded claims. Flow references are prose only. |
| `/deviate-release` | Release Planner (optional `product` pack) | `specs/_product/release-next.md` | (none — manual slash command only) | Goal-first slice of flows and work plus testable Acceptance Criteria; override semantics with `RELEASE_OVERRIDE`. Recommends `/deviate-explore` as next step; never auto-advances. |
...
**TDD mechanical evidence gate:** `HandoverManifest.evidence` is a first-class list of nested citations (`ac`, `test_path`, `test_quote`, `impl_path`, `impl_quote`) in `src/deviate/core/agent.py`. `_run_judge_phase` and `_apply_judge_verdict` validate that schema (YAML parse + `EvidenceItem` shape + `parse_errors`) **before** coerce / unmatched-PASS rewrite / revert. Evidence `parse_errors` (string items, missing required keys) are `JUDGE_MANIFEST_INVALID`: keep GREEN, retry JUDGE in auto (cap 3), refuse on `judge post`. They are not a compliance rejection of GREEN (GH-167). After a valid schema and `_coerce_judge_action`, TDD `_run_judge_phase` (`src/deviate/cli/micro.py`) resolves this task's required `AC-PLAN-NNN` tokens via `resolve_task_ac_tokens` (`src/deviate/core/judge_evidence.py`) and passes that list as `required_tokens` to `evaluate_judge_evidence`. First hit wins: non-empty `TaskRecord.acceptance_criteria` `criterion_id`s; else `AC-PLAN-NNN` tokens named in this task's `tasks.md` card — prefer `**Acceptance Criteria**` lines when present, otherwise scan remaining card text after dropping `**Rationale**`, `**Judge Feedback**`, and similar non-AC sections and their continuation lines (GH-191); else no AC tokens. Auto `_build_auto_prompt("judge")` injects that same stripped card as `{task_content}` (GH-118 / GH-150); `_task_card_text` still returns the raw card for token resolution, RED/GREEN feedback assembly, and file-list parsing. Sibling cards are never injected. The gate does not fall back to every token in `<authoritative_acceptance_contract source="plan.md">`. Omitting a later-shard plan token is legal at JUDGE. Auto and manual judge prompts require `evidence` only for the resolved task tokens. Quotes must copy from the already-built `<diff>` (`git diff <red>^..HEAD` where `<red>` is `_resolve_judge_diff_base(session.red_commit_sha)` — the RED-phase failing-test commit after walking back through `docs(...): add judge feedback for retry` subjects — plus dirty `git diff HEAD` and untracked `--no-index` hunks) or allowed HEAD files. On a clean `COMPLIANCE_PASS` (no `COMPLIANCE_VIOLATION` / `COMPLIANCE_FAIL` token, no structured Test Integrity fail), `_rewrite_unmatched_tdd_pass` returns the coerced forward action unchanged and does not log `JUDGE_EVIDENCE_REJECTED` (GH-185). Messy quotes (indentation / GH-178) are ignored on pass — they are not a reason to discard GREEN. A `REFACTOR NOTE` is still just a note for REFACTOR. `skip_refactor` / `proceed_to_refactor_no_diff` stay the same forward routes; they are not expanded. `COMPLIANCE_VIOLATION` still uses `revert_green` / `revert_red` as declared. When the judge already emitted `train_feedback` or `violations` on a violation route, persist that text (after the GH-103 citation strip) instead of replacing it with the generic missing-evidence string (GH-102). `skip_refactor` on the already-exists path may quote HEAD file contents for this-task tokens. A named test file absent on disk fail-closes at the COMPLETED write (`COMPLETED_EVIDENCE_MISSING`), not by rewriting a clean pass to `revert_green` (GH-185). Empty declared files remain a RED defect (`_require_tdd_declared_regression_files`). Tasks with no resolved `AC-PLAN-*` tokens may emit empty evidence quotes. `COMPLIANCE_VIOLATION` still uses `revert_green` / `revert_red` as declared. After the gate returns no feedback and the action is a completion path (`skip_refactor` / bare `COMPLIANCE_PASS` / post-REFACTOR complete / adjudicated already-exists), `_append_status_transition(..., "COMPLETED")` copies the validated `HandoverManifest.evidence` onto that COMPLETED `TaskRecord` as `evidence.items` and stamps `red` / `green` / `head` from `session.red_commit_sha` and `HEAD` (GH-84). After a clean `COMPLIANCE_PASS` (pending `continue_refactor` / `skip_refactor` / `proceed_to_refactor_no_diff`), `_require_tdd_completed_evidence` fail-closes only when this-task `AC-PLAN-NNN` tokens exist and the persisted bundle is missing/empty, a required this-task token has no evidence row, or a named `test_path` is absent on disk. It does not re-run `evaluate_judge_evidence(..., use_head=True)` exact-quote matching: REFACTOR and `_run_format_cmd` may edit cited files (indent, format, `impl_path` move) before COMPLETED, and JUDGE quotes persist as historical (GH-191 leftover). Plans with no `AC-PLAN-*` tokens may complete with empty evidence. The COMPLETED evidence gate runs only on the first COMPLETED write; `_append_status_transition(..., "COMPLETED")` is a no-op when the task is already COMPLETED (GH-146). EXECUTE, IMMEDIATE, and DIRECT judge paths stay ungated.
...
fresh RED record appends rather than rewriting the previous one — and the
JUDGE payload already on `session.train_feedback` (normalized `violations` /
`train_feedback`) is threaded into the retry RED prompt. A GREEN pytest dump
is replaced with a short `previous cycle failed because …` note; that note
is the fallback only when the payload is empty. `TRAIN_EXHAUSTED` prints only after three RED escalates.
...
`HandoverManifest`. Pi uses print mode (`pi -p`) and accepts the
...
`_invoke_agent` logs `AGENT_ERROR` with those tokens. See
