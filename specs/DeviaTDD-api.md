...
* **Execution Modes:**
  * **Offline Mode (Default):** Scans root project files using regex to resolve `${VARIABLE}`
    placeholders in constitution boilerplate. Completes in L_max <= 50ms.
  * **Onboard Prompt Mode:** `deviate init --generate-constitution` invokes an authorized LLM
    runner via the configured agent backend to analyze project state and generate a tailored
    constitution.
...
* **Input Parameters:**
  ...
  * `--generate-constitution` (Flag: triggers LLM runner)
...
* **Common Flags:** All `deviate init` and constitution commands accept `--json` (emit
  results as JSON to stdout) and `--quiet` (suppress diagnostic output).

#### `deviate constitution pre`

* **Source:** `src/deviate/cli/__init__.py`
* **Description:** Validates that `specs/constitution.md` exists and emits a JSON contract
  with `constitution_path`, `test_command`, `lint_command`, `typecheck_command` extracted
  from the constitution.
* **Common Flags:** `--json` (emit JSON contract to stdout), `--quiet` (suppress output)

#### `deviate constitution post <manifest>`

* **Source:** `src/deviate/cli/__init__.py`
* **Description:** Validates the constitution against required sections, runs pre-commit
  hooks, commits with `docs: update specs/constitution.md`, and saves the session.
...
* **Description:** Validate `explore.md` output. Reads the artifact, validates required
  ...
  create explore.md`, and saves the session. After successful commit, automatically runs
  `deviate context post` to synchronize agent context files.
...
* **Description:** Scans for constitutional violations, validates both `design.md` and
  ...
  and saves session. After successful commit, automatically runs `deviate context post`.
...
* **Description:** Loads the manifest JSON, validates `prd.md` sections against
  ...
  runs pre-commit hooks, commits, and saves session. After successful commit, automatically
  runs `deviate context post`.
...
* **Description:** Validates shard output (YAML frontmatter, empty files), registers each
  issue as `BACKLOG` status in `issues.jsonl`, runs pre-commit hooks, resets session to
  `IDLE`, and auto-runs `deviate context post`.

#### `deviate adhoc pre <task-description>`

* **Source:** `src/deviate/cli/macro.py`
* **Description:** Compressed fast-path for low/medium complexity tasks. Runs a complexity
  gate evaluation before proceeding. On acceptance, performs proportional lightweight
  codebase exploration, emits a JSON contract with `next_ADH_num`, `adhoc_dir`, and
  `prd_path` for the agent to synthesize a single vertical-slice issue.
* **Complexity Gate:**
...
* **Description:** Validates the issue markdown, appends a condensed FR entry to
  ...
  identifier, runs pre-commit hooks, commits, and auto-runs `deviate context post`.
...
### 5. Automated Sandbox Orchestration
...
* **Description:** Triggers the automated execution cycle for a single task node. Routes by
  ...
  cycle. Non-TDD (DIRECT, E2E) runs `_run_execute_phase` which immediately marks COMPLETED.
* **Input Parameters:**
...
* **Description:** Finds all PENDING tasks and dispatches them sequentially. Each task gets
  up to 2 retry attempts before being marked FAILED. Halts on first failure.
...
#### `deviate tasks list [--type tdd|direct|e2e] [--status <status>]`
...
#### `deviate issues list [--type feature|adhoc] [--status <status>]`
...
### 7. Agent Context Synchronization

#### `deviate context pre`

* **Source:** `src/deviate/cli/micro.py`
* **Description:** Performs an ascending directory crawl to find the project root and
  `.deviate/config.toml`. Discovers the current git branch name, derives the active feature
  slug, computes absolute paths to `CLAUDE.md`, `AGENTS.md`, `specs/constitution.md`, and any
  active `spec.md`/`design.md`. Formats all discovered data into a JSON contract (see output
  schema below).
* **Common Flags:** `--json` (default: outputs JSON contract to stdout), `--quiet`

#### `deviate context post <manifest>`

* **Source:** `src/deviate/cli/micro.py`
* **Description:** Reads the manifest with updated context values. Performs block replacement:
  finds the `## Technical Execution Context` block in `CLAUDE.md` and replaces it with compiled
  context from `spec.md` + `constitution.md`. Enforces `AGENTS.md` → `CLAUDE.md` symlink
  via `ln -sf`. Stages all changes and commits with
  `docs({NNN}): sync agent context`.

**Context JSON Contract Schema** (emitted by `deviate context pre --json`):
```json
{
  ...
  "claude_md_path": "/workspace/project/CLAUDE.md",
  "agents_md_path": "/workspace/project/AGENTS.md",
  "language": "Python",
  "multilang_mode": false,
  "test_command": "mise run test",
  "lint_command": "mise run lint",
  "typecheck_command": "mise run check-types",
  "dry_run": false,
  "plan_target": "/workspace/project/specs/auth-jwt-refresh/plan.md"
}
```

**Auto-Trigger:** The following `post` commands automatically invoke `deviate context post`
after their artifact commit: `explore post`, `research post`, `prd post`, `shard post`,
`adhoc post`, `specify post`, `tasks post`. Pass `--no-context-sync` to any of these to
skip the auto-trigger.
...
The `CacheDiscipline` module in `src/deviate/core/cache_discipline.py` provides
...
└── {FEATURE_SLUG}/           # Feature workspace bucket
    ...
    ├── spec.md               # Functional contract ("What & Why" system bounds)
...
│   ├── _common.py            # Shared helpers (_halt, _extract_epic_num, etc.)
│   ├── macro.py              # explore, research, prd, shard (pre/post)
│   ├── meso.py               # specify, tasks, pr (pre/post/run)
│   └── micro.py              # red, green, yellow, judge, refactor, execute, e2e, hotfix, run
├── core/
│   ├── agent.py              # AgentBackend, HandoverManifest
...
│   ├── auto/                 # RED.md, GREEN.md, YELLOW.md, JUDGE.md, REFACTOR.md
│   ├── governance/           # claudemd_seed.md, agents_seed.md
│   └── skills/               # 18 DeviaTDD skill directories
└── state/
    ├── __init__.py
    ├── config.py             # DeviateConfig, SessionState, TransitionViolationError
    └── ledger.py             # IssueRecord, TaskRecord, ledger operations
```
...
| --- | --- | --- | --- | --- |
| `/deviate-explore` | Context Scanner (Cheap) | `specs/{FEATURE_SLUG}/explore.md` | `deviate feature create`, `deviate explore pre/post`, `deviate context pre/post` | 6 steps: feature create, constitution validate, bucket allocate, codebase scan, write explore.md, commit + context sync |
| `/deviate-research` | Architect (Expensive) | `specs/{FEATURE_SLUG}/design.md`, `data-model.md` | `deviate research pre/post`, `deviate context pre/post` | 5 steps: read explore.md, analyze options, produce design.md, produce data-model.md, commit + context sync |
| `/deviate-prd` | Product Owner Proxy | `specs/{FEATURE_SLUG}/prd.md` | `deviate context pre`, `deviate prd pre/post` | 4 steps: read design.md, synthesize requirements, write prd.md, commit + context sync |
| `/deviate-shard` | Decomposition Engine | `specs/{FEATURE_SLUG}/issues/{ISS-NNN}-*.md` | `deviate shard pre/post`, `deviate context pre/post` | 5 steps: read prd.md, identify vertical slices, validate granularity, create issue stubs, register in ledger + context sync |
| `/deviate-adhoc` | Condensed Scoper | `specs/adhoc/` | `deviate adhoc pre/post`, `deviate context pre/post` | 8 steps: complexity gate, codebase scan, PRD append, issue generation, ledger registration, commit + context sync |
| `/deviate-specify` | Systems Architect | `specs/{FEATURE_SLUG}/issues/{ISS-NNN}/spec.md` | `deviate specify pre/post` | 3 steps: claim issue + create worktree, produce spec.md with Gherkin, validate + commit |
| **[HITL GATE]** | Human Reviewer | --- | --- | Review spec.md for completeness and edge cases before `/deviate-tasks` proceeds |
| `/deviate-tasks` | Technical Lead | `specs/{FEATURE_SLUG}/issues/{ISS-NNN}/tasks.md` | `deviate tasks pre/post` | 6 steps: resolve spec.md, decompose into tasks, assign execution modes, encode DAG deps, append terminal E2E task, validate granularity + commit |
...
- **Macro layer** (explore -> research -> prd -> shard): Sequential CLI invocations, each
  persisting session to `.deviate/session.json`. Phase transitions validated by `SessionState`.
- **Meso layer** (`/deviate-specify` -> `/deviate-tasks`): Single continuous LLM session. The system prompt,
  tool definitions, and `spec.md` content form a stable prefix cached after turn 1.
- **Micro layer** (RED -> GREEN -> REFACTOR): Task execution reuses the same in-process state
  via `force_transition_to()`. The `deviate run <task-id>` command dispatches through the
  full cycle programmatically.
...
| `status` | Literal | `PENDING`, `RED`, `GREEN`, `REFACTOR`, `COMPLETED`, `FAILED` |
