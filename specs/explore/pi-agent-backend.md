...
- **Best Practices — Print Mode Simplicity**: For single-shot, stateless invocations matching DeviaTDD's current model, `pi -p "<prompt>"` exits after the first assistant turn and streams the response to stdout. Source: `https://www.npmjs.com/package/@earendil-works/pi-coding-agent`. This is the lowest-friction integration: it maps to `BACKEND_COMMANDS["pi"] = "pi -p"`. Pi print mode accepts the `--model <id>` flag directly (e.g. `pi --model minimax/MiniMax-M3`), so the `provider/model` shorthand from DeviaTDD's `[models]` config passes through verbatim — same as `opencode` and `droid`.
