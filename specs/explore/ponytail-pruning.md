...
- The prompt template tests that pin "ponytail smallest-change" (`tests/test_meso/test_auto_prompt_templates.py`).
- The PR/MR test coverage (`tests/test_meso/test_pr_platform.py`, `tests/test_cli/test_meso.py`, `tests/test_cli/test_meso_contracts.py`, `tests/test_integration/test_meso_layer.py::TestPrRun`).
...
- **Quality, Safety & Observability**: pytest under `tests/` with per-domain dirs (`tests/test_meso/`, `tests/test_cli/`, `tests/test_integration/`), ruff lint, `bats` E2E, and a review-coverage module `src/deviate/core/review_coverage.py` used by `src/deviate/cli/review.py::pre`.
...
| `tests/test_meso/test_pr_platform.py` | Test | Pins platform detection and GitLab push-option generation. | "def test_github_remote(self, tmp_path):\n    with patch(\"deviate.cli.meso.subprocess.run\") as mock_run:\n        mock_run.return_value.stdout = \"https://github.com/owner/repo.git\\n\"\n        assert _resolve_pr_platform(tmp_path) == \"github\"" |
| `tests/test_cli/test_meso.py` | Test | Pins the conventional-commit PR title generation. | "assert _pr_title(\"ISS-001-001\", \"Feature\") == \"feat(001-001): Feature\"\nassert _pr_title(\"ISS-ADH-001\", \"Fix\", \"bug\") == \"fix(ADH-001): Fix\"" |
| `tests/test_cli/test_meso_contracts.py` | Test | Pins the `deviate pr pre` JSON contract fields. | "def test_pr_pre_contract_has_required_fields(self, tmp_path: Path) -> None:\n    ..." |
...
| `tests/test_meso/test_auto_prompt_templates.py` | Test | Pins the "Ponytail smallest-change" decision: folded into existing GREEN / REFACTOR / review lines, no new Constraints or Minimality heading. | "class TestSmallestChangeFoldedIntoExistingPrompts:\n    \"\"\"GH-92 (rescoped): Ponytail smallest-change lives in existing GREEN /\n    REFACTOR / review lines — no new Constraints or Minimality heading.\"\"\"" |
| `tests/test_meso/test_auto_prompt_templates.py` | Test | Pins review behavior: keep over-engineering signals, do not promote helper extraction. | "def test_review_keeps_overengineering_and_does_not_promote_helpers(self):\n    text = self._read_review()\n    assert \"Cross-task over-engineering\" in text\n    assert \"into a shared helper\" not in text\n    assert \"## Constraints\" not in text" |
...
| `CHANGELOG.md` | Config | Records the `/deviate-pr` PR/MR behavior change with pinned tests. | "- **`/deviate-pr` now marks the issue COMPLETED before it pushes, and opens a PR/MR only on request.** ... Pinned by `tests/test_integration/test_meso_layer.py::TestPrRun` and `tests/test_meso/test_pr_platform.py`." |
...
| Files Likely Modified | 2-5; strongest candidates: `src/deviate/prompts/commands/deviate-review.md` (if ponytail pruning folds in), or a new `src/deviate/prompts/commands/deviate-ponytail.md` (if a new command is created), plus `tests/test_meso/test_auto_prompt_templates.py`. The `/deviate-pr` concern is already implemented; any change is verification or a small fix in `src/deviate/cli/meso.py` and `tests/test_meso/test_pr_platform.py`. |
