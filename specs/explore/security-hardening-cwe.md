...
| `tests/test_state/test_security_profile.py` | Test | Pins the SecurityProfile ledger contract (5 tests) | `class TestSecurityProfile:` / `    def test_security_profile_default_construction(self):` / `        """Empty SecurityProfile() yields body=None."""` |
| `tests/test_micro/test_judge.py` | Test | Pins the security_checks manifest field requirement | `class TestJudgeSecurityChecksField:` / `    """The JUDGE prompt must declare 'security_checks' as a required manifest field.` |
