...
- frontmatter is valid Tome schema (`title`, `description`, `doc_type`, `status`, `last_verified_at`, `verified_sha`, `related_issues`, **`prev`**, **`next`**) — `prev`/`next` MUST be set per the classifier's `parent`/`next` fields (`false` when the page is a quadrant intro, the first page in a theme, or the last page in a theme). The writer contract uses the literal `false` rather than `null` because Starlight's `docsSchema()` (which powers the Astro content-collection validation at build time) accepts `boolean | string | {link, label}` for these fields and rejects `null`. Semantically `false` and `null` both mean "no link"; the verifier (C6) treats them as equivalent in its IA reconciliation.
...
- **Frontmatter schema** (inlined in each writer prompt; nine fields, two of them new in this iteration):
  ...
  related_issues:
    - ISS-123
  prev: <repo-relative path | false>    # IA: prior page in the reading order; false for quadrant intros and the first page in a theme
  next: <repo-relative path | false>    # IA: next page in the reading order; false for the last page in a theme
  ---
  ```
...
- **IA reachability** (new in this iteration): every updated file under a quadrant MUST appear in that quadrant's per-theme `<quadrant>/<theme>/_meta.yml` ordering (or in the per-quadrant `<quadrant>/_meta.yml` for files at the quadrant root); every page MUST carry `prev`/`next` frontmatter (or `prev: false` if it is the first page in its theme; or `next: false` if it is the last; Starlight's docsSchema() rejects `null`, so the writer contract uses `false` as the sentinel). The per-quadrant `<quadrant>/index.md` (one per quadrant) and the root `apps/docs/src/content/docs/index.md` are exempt from `prev`/`next` checks (they are navigation pivots, not sequence members). The per-quadrant `<quadrant>/_meta.yml` MUST have `index.md` as the first entry of its `pages:` list (the `index-first` ordering invariant); any other order is a `[FAIL-INDEX-FIRST]` finding. A legacy `<quadrant>/intro.md` file (if present) is also a `[FAIL-INDEX-FIRST]` migration candidate — the developer runs `/tome-setup` to migrate it to `<quadrant>/index.md`.
