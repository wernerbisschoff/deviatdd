...
The design keeps all state in the existing append-only JSONL ledger protocol and the `.deviate/` session store. It adds no persistent database runtime, respecting the constitution's "No persistent database runtime" clause. The `TaskRecord.status` Literal stays unchanged; the new checkpoint and traceability use additive fields, so the existing canonical-state derivation by sequential ledger parsing remains valid. The `AO-NNN` → `AC-PLAN-NNN` → task → test chain is extended, not replaced, so `specs/DeviaTDD-api.md` and `specs/DeviaTDD-architecture.md` update in the same commit.

**Module Surface:**
- **Modify** `src/deviate/cli/micro.py` — RED non-blocking checkpoint; GREEN blocking enforcement; REFACTOR blocking regression gate.
- **Modify** `src/deviate/state/ledger.py` — additive `acceptance_criteria` field on `TaskRecord`; persistent Red-checkpoint record helper.
...
| Persistent Red-checkpoint stored in `.deviate/` session store | Non-blocking pass-through vs. durable checkpoint record | Keeps the checkpoint out of the append-only task ledger so a non-terminal phase does not pollute `tasks.jsonl`; session state already lives in `.deviate/` per constitution §2 |
| GREEN becomes blocking on test failure | Hard failure (reject task) vs. current soft feedback (`micro.py:1298-1311` sets `train_feedback` and returns) | "GREEN phase must pass all tests" is a constitution Testing Protocol clause; explicit blocking enforcement makes the DoD enforceable |
...
- **(Persistent checkpoint in `.deviate/`)** — A crash between RED and GREEN could lose the checkpoint state. Mitigation: write the checkpoint atomically via the existing `SessionState` persist path.
...
| RSK-003 | Non-blocking RED checkpoint permits a passing-test RED to slip through | M | M | Checkpoint records a warning; GREEN gate remains the hard validator | micro | `src/deviate/cli/micro.py:1123` |
| RSK-004 | Checkpoint state lost on abrupt process exit between RED and GREEN | L | M | Persist via existing `SessionState` atomic save | micro | `.deviate/session.json` |
...
| :--- | :--- | :--- | :--- |
| "GREEN phase must pass all tests" (§3) | GREEN becomes a blocking validation phase | Aligned | Direct enforcement of the clause |
...
|---|---|---|---|---|---|
| `HITL-001` | Where is the persistent non-blocking RED checkpoint stored? | explore.md Scope Sizing defers to "in `.deviate/` or the task ledger" (SRC-003). | `.deviate/` keeps `tasks.jsonl` clean of non-terminal records; the task ledger makes the checkpoint durable and auditable but violates the append-only semantics for a non-terminal phase. | Store the checkpoint in `.deviate/` session state; keep the task ledger strictly for terminal/phase transitions. | `PENDING` |
| `HITL-002` | Should the GREEN phase reject the task (raise `PhaseFailedError`) on test failure, or keep the current soft `train_feedback` return and only record a blocking outcome? | Current GREEN at `micro.py:1298-1311` holds the implementation for JUDGE on test failure. explore.md proposes "GREEN and Refactor blocking validation phases". | Hard rejection aborts the slice; soft blocking records the failure and lets JUDGE decide. | Make GREEN record a blocking FAILED verdict on test failure but let JUDGE assess the partial implementation, matching the existing `train_feedback` design. | `PENDING` |
| `HITL-003` | How is verification-mode (`automated` / `manual` / `deferred`) encoded on an `AC-PLAN-NNN` scenario? | Explore proposes "explicit verification-mode metadata" on the existing contract; no format is specified. | A structured field (e.g., `**Verification Mode**: automated`) is human-readable and Gherkin-safe; a front-matter table is harder to validate. | Encode it as a per-scenario `**Verification Mode**: <mode>` line inside each `AC-PLAN-NNN` scenario body. | `PENDING` |
| `HITL-004` | Does the non-blocking RED checkpoint require a new `TaskRecord` status value, or a separate checkpoint record? | `TaskRecord.status` Literal is fixed at `PENDING/RED/GREEN/JUDGE/REFACTOR/COMPLETED/FAILED` (`ledger.py:85`). | A new status mutates the canonical state machine and DoD; a separate checkpoint record avoids it. | Keep the status Literal unchanged; record the checkpoint in `.deviate/` session state. | `PENDING` |
