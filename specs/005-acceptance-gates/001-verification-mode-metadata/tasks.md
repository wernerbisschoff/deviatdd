...
- TSK-005-01: Mode-literal validation in `validate_acceptance_contract` plus unit tests
  ...
  - **Details**:
    ...
    - **Acceptance**: `uv run pytest tests/test_core/test_validation.py -v` passes with the named error strings pinned verbatim in the reject assertions. Existing clause tests still pass. `validate_gherkin_syntax` remains mandatory.
...
- TSK-005-02: Add the Verification Mode line to seeded meso plan fixtures
  ...
  - **Mode**: IMMEDIATE
  - **Verification**: `uv run pytest tests/test_meso/test_meso_orchestration.py tests/test_meso/test_meso_resume.py tests/test_meso/test_plan_structure_injection.py -v`
...
- TSK-005-03: Gate-level regression tests for mode-less and illegal-mode contracts
  ...
  - **Verification**: `uv run pytest tests/test_cli/test_meso_contracts.py tests/test_meso/test_meso_resume.py -v` then `mise run check`
  - **Estimated Time**: 60 minutes
  - **Flow References**: []
  ...
  - **Details**:
    ...
    - **Acceptance**: `uv run pytest tests/test_cli/test_meso_contracts.py tests/test_meso/test_meso_resume.py -v` passes. `mise run check` (lint, format-check, types, full suite) exits 0. `src/deviate/cli/meso.py` carries no diff.
...
**Risk Hotspots**:
- The plan's Workstation Mapping does not list `tests/test_meso/test_meso_orchestration.py` or `tests/test_meso/test_plan_structure_injection.py`, yet both seed contracts that fail validation after the gate lands. TSK-005-02 covers this gap; without it, `mise run check` fails.
...
- `tests/test_meso/test_meso_resume.py` is touched by TSK-005-02 (edits `VALID_PLAN`) and TSK-005-03 (adds a new test method). The edits do not overlap; the tasks run sequentially.
- `tests/test_core/test_validation.py` is touched only by TSK-005-01.
