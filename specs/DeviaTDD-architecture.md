...
* **`/deviate-prune` (operational, post-COMPLETED cleanup):** Single surface for honeycomb test thinning plus that COMPLETED issue's drop-safe cycle markdown. `deviate prune pre` / `post` (`src/deviate/cli/prune.py`, engine `src/deviate/core/prune.py`) resolve one issue per invocation. Drop tests tagged `spy` / `impl` (name or marker); keep `behavioral` / `ac`. After COMPLETED, delete only per-issue `plan.md`, `tasks.md`, leftover `design.md` / `data-model.md`, then the folder if empty. Halt when plan ACs are not yet encoded as behavioral / `ac` tests. In-flight issues are a no-op for spec deletion. Never compact, rewrite, or delete `specs/issues.jsonl`, `specs/**/tasks.jsonl`, or `specs/_product/flows.jsonl`. Keep epic `explore.md` / `prd.md`, shared `specs/adhoc/prd.md`, and `specs/**/issues/*.md` (judge still globs `Module:` lines; `shard_post` re-reads the issues dir). Does not change Gate 1, explore, research, review, or ponytail-in-review.
...
| `/deviate-prune` | V4 Flash | Single invocation (one COMPLETED issue) | One-shot |
