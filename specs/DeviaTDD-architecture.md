...
boundaries.** Execution profile (`--profile [full|fast|judge]`) determines which phases are
...
6. **The Elastic Governance Rule:** The `deviate micro run` command (and the
   ...
   `--profile [full|fast|judge]` to control which phases execute. `full` runs
   the complete RED → GREEN → JUDGE → REFACTOR cycle. `fast` runs RED + GREEN
   only (skip JUDGE + REFACTOR). `judge` runs RED + GREEN + JUDGE (skip
   REFACTOR). Legacy `secure` coerces to `judge`. Boolean `--no-judge`/`--no-refactor` flags are retained as
