...
7. **Atomic Concurrency Protocol (Git Reference Locks):** To eliminate TOCTOU race conditions across distributed terminal instances, the issue claim workflow (formerly `deviate specify pre`, now part of the Plan phase orchestration) uses try-claim semantics. `select_unblocked_candidates()` returns all available BACKLOG issues. The worker iterates through them and attempts `claim_issue()` combined with `create_worktree()`. The default serialization is push-as-lock: `git push -u <remote> <branch>`. The server serializes concurrent pushes. The first successful push wins. Push-as-lock is the default, not a mandatory step. Local mode (`--local` on `deviate specify`, `deviate meso run`, or `deviate run`, or `claim_remote = false` in `.deviate/config.toml`) keeps the worktree and the ledger claim and skips the remote lock. The `tasks.jsonl` ledger records the authoritative outcome.
