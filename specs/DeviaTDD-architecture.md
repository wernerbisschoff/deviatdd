...
* **Plan (`deviate plan pre` / `deviate plan post`):** Per-issue localized research phase
  ...
  Tasks emits a trackable execution blueprint in `specs/{FEATURE_SLUG}/issues/{ISSUE_ID}/tasks.md`; post validates and commits it, then `deviate run` chains directly into `deviate micro run --all`. Each entry has a unique `TSK-{issue_suffix}-{NN}` identifier (`issue_suffix` is the last numeric component of `issue_id`: `001-004` → `TSK-004-01`, never the epic prefix), type (`tdd`, `direct`, or `e2e`), **Test Strategy** `unit` | `integration` | `e2e` (required on TDD; `Sociable_Unit` / `Solitary_Unit` are retired runner values), file locations, mock boundaries, and fixture requirements. Type `Verification_Batch` is locked to `execution_mode: IMMEDIATE` (EXECUTE path — never TDD); `src/deviate/core/tasks_ledger.py::resolve_execution_mode` enforces the mapping when parsing `tasks.md`. `parse_test_strategy` persists `unit` | `integration` | `e2e` onto `TaskRecord.test_strategy`. One TDD task names exactly one layer; `deviate tasks post` rejects a TDD card with two stamps, two layer write dirs, or two layer Verification commands (`MIXED_TEST_LAYER`, `src/deviate/core/tasks_ledger.py::validate_tdd_task_layers`) and does not commit. Micro/RED dispatch does not fail-closed on historical mixed cards. A closing sweep is emitted only when a rung exists: user-facing + e2e → terminal `[E2E]` `Verification_Batch` whose Verification is the full existing ladder; else if integ exists → `[VERIFY]` unit-if-exists + integ; else no extra sweep. Never emit empty e2e files; never require integ to be set up. Issues touching only library/config internals with no user-facing surface omit the closer.
...
with `MIXED_TEST_LAYER` (split into two TDD tasks). Micro dispatch does not fail-closed
on historical mixed cards; injected `test_strategy` / `test_write_dir` / `test_command`
stay one layer. IMMEDIATE `[VERIFY]` / `[E2E]` closing sweeps may still name
