...
- The **Green → Judge → Green loop** is the JUDGE → GREEN arrow at the top of
  ...
  `GREEN_ENTRY_REFUSED` when it is still missing, including when
  `current_phase` is already GREEN. The `deviatdd` skill classifies that
