...
[ MICRO LAYER: TDD Loop ] ──> Red ──> Green ──> Judge/Train ──> Refactor
              ...
              │  injected (3 GREEN runs then escalate;           │
              │  TRAIN 3/3 is a real GREEN)                      │
              │                                                   │
...
* **Explore (Cheap Context Gathering):** Fast, inexpensive scan of codebase structure, dependencies, existing patterns, and tech stack. Runs on a cheap model (V4 Flash). Outputs raw factual context to `explore.md` — what exists, not what to do. When a nearest existing user flow exists, catalog a short sibling-flow inventory (amount vs fee, lock vs reserve, vendor call in HTTP vs job, idempotency, destination shape) with quoted paths and no recommendations. When an open/local epic bucket's problem or domain overlaps the statement, catalog it under `## Related Epic Candidates` (path, title, evidence quotes; `None observed` when none) — still factual, no architecture recommendations. Status Summary `NEXT_ACTION` is one of `attach_existing_epic <NNN-slug>`, `new_epic` (`/deviate-research`), or `adhoc` (`/deviate-adhoc`); a human can force the route via `HITL_OVERRIDE`, `## Pending HITL Decisions`, or explore `user_input`. `deviate explore post` persists those fields on the session. On attach, `deviate research pre` / `deviate adhoc pre` skip `allocate_feature_bucket` and the shared adhoc PRD — the later issue writer lands under that epic's `issues/` and reuses the epic PRD. Ecosystem research stays catalog only. Initial output lands in `specs/explore/<slug>.md`; `deviate research pre` **moves** that file into the numbered epic directory at `specs/{NNN}-<slug>/explore.md` so every artifact for one feature lives under a single directory. `allocate_feature_bucket()` numbers an unnumbered slug from `max(local numbered specs dirs ∪ remote feat/<NNN>-* prefixes) + 1`; a numbered slug stays idempotent. The move is recorded as a tracked rename (usually rendered as `R100` in `git show`) inside the same atomic commit that `deviate research post` produces for `design.md` + `data-model.md` — the source path `specs/explore/<slug>.md` is `git rm`-ed and the destination `specs/{NNN}-<slug>/explore.md` is `git add`-ed, so the working tree never carries a tracked-but-deleted or untracked residue from the move. The source path is persisted on `SessionState.research_explore_source` at pre time so the post-script can `git rm` the right path even when `allocate_feature_bucket` short-circuits on an already-numbered slug.
* **Research (Architectural Design):** Consumes `explore.md` (at `specs/{NNN}-<slug>/explore.md` after the research pre move) and performs high-level architectural analysis as a **range**, not a maxed design. One agent writes the floor (constitution + sibling parity + authorization/money safety/provider correctness/data integrity) then attacks it in the same prompt — no sequential AlphaBeta/Gamma sub-agent spawns. Schema tables are floor only; named extras live in a Deferred / maximal bracket list. Existing constitutions stay read-only except greenfield bootstrap or an explicit HITL amendment. Runs on a reasoning model (Qwen thinking or V4 Pro). Outputs `design.md` (architecture and decisions) and `data-model.md` (entity relationships, schemas, and request-flow / runtime `## Data Flow`), both inside the same numbered epic directory. `## Data Flow` stays named `Data Flow` for validator stability and is an arc42 Runtime view: primary scenario prose, a fenced Mermaid `sequenceDiagram` (or `None — local-only` when explore proves zero externals), an Outbound integrations table (or the same marker), between-hops reliability, and an alternate/failure path. Schema-only Data Flow fails `research post`. `design.md` `## Recommended Architecture` cross-links to that section. PRD Functional Flow reuses or refines the research sequence rather than inventing the first one.
...
* **Shard:** Breaks down the PRD into standalone technical issue files (GitHub Issues). Each issue must be a **vertical slice** — a complete, testable behavior end-to-end (not a horizontal layer like "add database"). Emit as few independently shippable user-visible verticals as the PRD needs, with no fixed count cap. Each issue must be independently implementable and testable, with clear acceptance criteria. Shard carries covered AC/AO pairs and maps each AC to an executable Verification Command. **Slicing rules live entirely in the `deviate-shard` prompt**; the PRD prompt never pre-decides groupings.
  * **Epic verification ownership:** SHARD reuses an existing command when sufficient. Otherwise, the earliest relevant behavior issue creates `mise run verify:<epic_slug>` with real checks for its FR/AO outcomes. No setup-only issue or empty passing placeholder is allowed. Issues requiring the command declare `blocked_by`. TASKS instructions and automatic runner execution remain unchanged.
...
* **Plan (`deviate plan pre` / `deviate plan post`):** Per-issue localized research phase
  ...
  The `pre` contract opens with a traceability gate (`validate_issue_traceability` in `src/deviate/core/validation.py`): `READY` passes, `NOT_READY` names each missing field in `missing_fields` with a `repair_hint`, `ISSUE_NOT_FOUND` covers a missing file. The gate checks section names only (`User Stories Ledger`, `Upstream Requirement Tracing`, `Acceptance Outline`, plus one `AO-NNN` token). Legacy issues recover via `repair_issue_traceability` (`src/deviate/cli/meso.py`), which inserts only absent sections under `specs/` and revalidates. Shard and adhoc templates emit these sections and token families so new issues pass without repair.
  Tasks emits a trackable execution blueprint in `specs/{FEATURE_SLUG}/issues/{ISSUE_ID}/tasks.md`; post validates and commits it, then `deviate run` chains directly into `deviate micro run --all`. Each entry has a unique `TSK-{issue_suffix}-{NN}` identifier (`issue_suffix` is the last numeric component of `issue_id`: `001-004` → `TSK-004-01`, never the epic prefix), type (`tdd`, `direct`, or `e2e`), **Test Strategy** `unit` | `integration` | `e2e` (required on TDD; `Sociable_Unit` / `Solitary_Unit` are retired runner values), file locations, mock boundaries, and fixture requirements. Type `Verification_Batch` is locked to `execution_mode: IMMEDIATE` (EXECUTE path — never TDD); `src/deviate/core/tasks_ledger.py::resolve_execution_mode` enforces the mapping when parsing `tasks.md`. `parse_test_strategy` persists `unit` | `integration` | `e2e` onto `TaskRecord.test_strategy`. One TDD task names exactly one layer; `deviate tasks post` rejects a TDD card with two stamps, two layer write dirs, or two layer Verification commands (`MIXED_TEST_LAYER`, `src/deviate/core/tasks_ledger.py::validate_tdd_task_layers`) and does not commit. Micro/RED dispatch fail-closes with `SPLIT_TASK_REQUIRED` before any agent spawn when a mixed contract is detected (ledger stamp plus card paths/commands). `deviate red refresh-layer --task` restamps `TaskRecord.test_strategy` from an operator-corrected card so a stale unit row cannot keep failing after a single-layer correction (GH-252). Keyword-only cards without concrete multi-layer targets keep the previous fallback. A closing sweep is emitted only when a rung exists: user-facing + e2e → terminal `[E2E]` `Verification_Batch` whose Verification is the full existing ladder; else if integ exists → `[VERIFY]` unit-if-exists + integ; else no extra sweep. Never emit empty e2e files; never require integ to be set up. Issues touching only library/config internals with no user-facing surface omit the closer.
...
* **GREEN (The Execution):**
    ...
    * **GREEN-entry invariant:** `_run_green_phase` (`src/deviate/cli/micro.py`) always persists a recoverable `session.red_commit_sha` then invokes the GREEN agent only when that SHA is a standing RED-phase failing-test boundary **for the active task**. `_bind_session_red_boundary` / `_recover_red_commit_boundary` reject a leftover SHA whose subject names a completed predecessor (GH-247). Skip-RED resume and existing-test / passing RED call `_ensure_red_ledger_transition` so the GREEN kernel sees `RED`, not `PENDING`. The gate runs even when `current_phase` is already GREEN. Empty SHA raises `PhaseFailedError` carrying `GREEN_ENTRY_REFUSED`. A `docs(...): add judge feedback` SHA that does not rest on a RED ancestor is recovered from ledger + git (GH-236: including after a non-fatal `ROLLBACK_FAILED` TRAIN; GH-240: `ROLLBACK_RECOVERY_HOOK_MISSING` / `ROLLBACK_RECOVERY_HOOK_FAILED` is `ENV_NOT_READY` and never trains) before the agent runs; if it is still unusable the runner raises `DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` rather than `GREEN_ENTRY_REFUSED` that prints the leftover SHA. After JUDGE `revert_red` / cycle `no_failing_test_adjudicated`, `_run_tdd_cycle` re-dispatches RED or raises `TRAIN_EXHAUSTED` / `PhaseFailedError`. It never enters GREEN on that path. C1 (`deviate` CLI) owns this gate. `skip_refactor` / bare `COMPLIANCE_PASS` still complete without GREEN. `_coerce_judge_action` and the 3/3 caps from ISS-ADH-017 stay unchanged.
    ...
    * **JUDGE human escalation:** JUDGE may immediately emit `status: ERROR` with `summary`, concrete `rationale`, and structured `contract_drift`, `escalates_to`, or `hitl_options`. Conflicts name both requirements and source references. Automatic JUDGE and manual `judge post` prioritize this signal over compliance routing and evidence repair. They preserve work, record `HITL_PENDING`, and raise `HitlEscalationError`. The micro chain stops without rollback, automatic retries, or training-budget consumption. The human corrects requirements or artifacts before explicitly resuming. No compliance verdict or evidence is required.
    * **Layer discipline:** GREEN's only invariant is "make the RED test pass via the library/API surface declared in scope." It does NOT make scope, spec-drift, or HITL-routing judgments — those belong to JUDGE. When a RED test cannot be satisfied within GREEN's mechanical scope, GREEN emits `status: FAILURE` with a concrete `rationale:` naming the test path and why; `status: "ERROR"` is reserved strictly for tool/orchestration failure. The runner's `_is_hitl_escalation` is a narrow defensive fallback that ONLY promotes structured `contract_drift` / `escalates_to` / `hitl_options` dict keys to `HITL_PENDING` — loose-string `error_kind` discriminators and free-form scope-conflict text do NOT trigger HITL escalation. Separately, `_apply_judge_verdict` may raise `HitlEscalationError` on contradictory successive JUDGE Requirement/Correction texts (`JUDGE_REQUIREMENT_CONTRADICTION`, GH-230) without an agent-supplied HITL payload.
...
* **JUDGE / TRAIN (The Compliance Gate) — with Green → Judge → Green loop:**
    * **Existing behavior (GH-242):** Automatic and manual JUDGE prompts inspect current task-scoped tests and implementation, including unchanged code.
      Diffs attribute phase changes; they do not define all available behavior.
      Valid existing tests and implementation satisfy assigned criteria without duplication. Irrelevant test renames do not establish coverage.
      Citations may reference inspected current files. Verdict routing, test gates, and evidence storage remain unchanged.
    * **Repair contract:** `auto/judge.md` defines rejection feedback within the existing `train_feedback` text field.
      Each confirmed defect includes Requirement, Evidence, Correction, Verification, and Boundary bullets.
      Feedback targets the next RED or GREEN phase and remains executable after rollback.
      GREEN preserves tests. RED must not edit production code. Neither phase expands the acceptance contract or requires later-task work.
      Automatic and installed manual prompts share this guidance. It adds no rejection gate, schema validation, or routing change.
    * **Resumed RED:** `_run_tdd_cycle_impl` calls `_consume_retry_gate_after_red` after its initial RED, matching `_escalate_to_new_red`. A non-empty RED commit boundary clears the previous JUDGE decision. The runner saves the cleared state before GREEN and preserves feedback. An empty boundary retains the retry decision.
    * **Phase attribution:** Auto JUDGE also receives `<green_diff>` from the GREEN-entry boundary through HEAD, including dirty and untracked changes. The combined `<diff>` keeps RED evidence. RED-only changes must not cause GREEN scope rejections. Runner-authored ledger transitions and feedback are not agent scope violations.
    * **The Judge:** The CLI evaluates the committed RED-parent-to-HEAD diff against `spec.md` for invariant/security violations. When `session.red_commit_sha` has advanced onto a `docs(...): add judge feedback for retry` commit or a meso docs artifact (`docs(...): create tasks.md` / `create plan.md`), `_assemble_judge_injected_diff` calls `_resolve_judge_diff_base` to walk those `docs(` subjects back to the RED-phase failing-test commit or the first non-docs commit before running `git diff {red_sha}^..HEAD`, so RED tests stay visible to the evidence gate (GH-88 / GH-90 / GH-201). Content diffs are captured as bytes and decoded with `errors='replace'` so latin-1 CSS cannot raise `UnicodeDecodeError`; real binary paths (PNG and other git-or-magic binaries) are replaced with a one-line stub (`Binary file <path> added|modified|deleted (<n> bytes)`) and are never dumped into the JUDGE prompt. `git diff --text` / `-a` is never used (GH-186). If GREEN tests failed before the implementation commit, `_run_judge_phase` also appends staged/unstaged `git diff HEAD` output and per-file `git diff --no-index /dev/null <path>` output for untracked files, so JUDGE assesses the retained implementation rather than a false RED-only snapshot. This judge operates in a clean, zero-shared-history session to break recursive subjectivity. A `deviate-judge` skill (loaded from `_SKILL_NAMES["JUDGE"]`) guides the agent through supplementary compliance evaluation. TDD JUDGE evidence is task-scoped: `_run_judge_phase` resolves required `AC-PLAN-NNN` tokens via `resolve_task_ac_tokens` (non-empty `acceptance_criteria` `criterion_id`s, else this task's `tasks.md` card preferring `**Acceptance Criteria**` lines and otherwise dropping `**Rationale**` / `**Judge Feedback**` / similar non-AC sections, else none) and does not fall back to every token in `plan.md`. Auto `_build_auto_prompt` injects only this task's card as `{task_content}` (GH-150). RED/GREEN/JUDGE cards exclude feedback; REFACTOR keeps the raw card. `_task_train_feedback` combines every complete recorded JUDGE round with any unrecorded session feedback into one RED/GREEN `<train_feedback>` section. `_train_feedback_placeholder` escapes `&`, `<`, and `>` before insertion, so literal closing tags cannot terminate the section. Stored feedback remains unchanged. `_build_auto_prompt` injects `<layer_lock>` above that section for RED, GREEN, and JUDGE so carry-forward cannot reclassify the stamped layer (GH-248). Earlier constraints remain active unless later feedback explicitly replaces them; the layer lock is not replaceable. Task acceptance and phase boundaries still apply. `_append_judge_feedback` retains every round and indents continuation lines, including nested Markdown. JUDGE filtering removes complete feedback blocks so prior corrections cannot alter acceptance scope (GH-118). `_task_card_text` remains raw. Existing orphaned feedback lines require explicit repair; the runner does not infer their ownership. TDD `_run_judge_phase` / `_apply_judge_verdict` validate the complete JUDGE manifest (YAML parse + `EvidenceItem` shape + `parse_errors`) **before** any verdict side effects. String evidence items or missing required keys recover as `parse_errors` and emptied `evidence` — that is `JUDGE_MANIFEST_INVALID` (retry JUDGE, keep GREEN), not a compliance rejection (GH-167). Auto retries inject the latest validation errors and `EvidenceItem.model_json_schema()` into one `<judge_manifest_repair>` block. The repair instruction requires complete YAML and object evidence, forbids fabricated citations, and preserves the GREEN tree. Each retry replaces that repair block; the cap remains three attempts. Exhausted single-task runs report the error and retry guidance with exit 1, without an uncaught traceback. No malformed evidence is coerced into a passing citation. After a valid schema, TDD `_run_judge_phase` then, on a clean `COMPLIANCE_PASS`, `_rewrite_unmatched_tdd_pass` returns the coerced forward action unchanged even when evidence quotes fail the substring gate (GH-185) — it does not log `JUDGE_EVIDENCE_REJECTED` and does not rewrite to `revert_green`. Messy quotes (indentation / GH-178) and a `REFACTOR NOTE` are not a revert. `COMPLIANCE_VIOLATION` still uses `revert_green` / `revert_red` as declared. On a forward PASS the runner stashes those citations on `SessionState.validated_evidence` (transient only) and writes them onto the COMPLETED `tasks.jsonl` row as `TaskRecord.evidence` (`items` plus `red` / `green` / `head` SHAs) so `deviate inspect tasks show` can display the proof after the session is gone (GH-84). After a clean `COMPLIANCE_PASS`, TDD COMPLETE with this-task `AC-PLAN-NNN` tokens refuse when that bundle is missing/empty, a required token has no evidence row, or a named `test_path` is absent on disk. It does not rematch frozen `test_quote` / `impl_quote` against post-REFACTOR HEAD (`evaluate_judge_evidence(..., use_head=True)`); `_run_format_cmd` stays before the COMPLETED write (GH-191 leftover). `.deviate/` session files are not the proof store. Earlier RED/GREEN/JUDGE rows stay lean. Persist the judge's own `train_feedback` / `violations` when present (GH-103 citation strip still applies); attach the generic missing-evidence string only when the judge emitted none (GH-102). `_append_judge_feedback` keys the card with `_TASK_BULLET_HEAD_RE` exact-id match and inserts under that card only (does not walk past a later phase heading). Gate 3 review (`deviate review`) owns plan-wide AC coverage. EXECUTE, IMMEDIATE, and DIRECT judge paths stay ungated.
    * **The Train (Green → Judge → Green loop):** On `COMPLIANCE_VIOLATION` or test failure, the CLI safely resets without destroying task progress. The JUDGE phase honors `HandoverManifest.next_action` (see [specs/DeviaTDD-api.md](./DeviaTDD-api.md) for the routing table). The five routes:
        1. **`revert_red`** — discard this task's GREEN **and** its RED. `_resolve_revert_red_boundary` classifies the stored SHA first (GH-168): a rebase-rewritten RED remaps by subject-match so `red^` stays on the current train; an already-reverted session (HEAD reset behind the stored SHA, or a feedback commit beside the discarded RED) no-ops onto current HEAD and does not raise `ROLLBACK_STALE_RED_SHA`. `_resolve_pre_red_sha()` then walks docs-feedback and meso docs `red_commit_sha` values via `_resolve_judge_diff_base` / `_WALKABLE_DOCS_SUBJECT_RE` / `_PRE_RED_SHA_PARENT_RE` (`docs(...): add judge feedback for retry`, `docs(...): create tasks.md`, `create plan.md`, and other `docs(` meso artifacts) to the RED-phase failing-test commit or the first non-docs commit, then returns that commit's parent (true pre-RED). A stored SHA equal to the tasks.md commit must not resolve to plan.md (GH-201). Logs `PRE_RED_AMBIGUOUS` only when that parent cannot be resolved. `revert_green` still resets to the TRAIN boundary (feedback or RED), not pre-RED. The pre-RED anchor is threaded into `_execute_rollback(boundary_sha=<pre_red>, task_id=<tid>, attempt=<rollback_attempts>)`; after `git reset --hard` + `git clean -fd`, `_execute_rollback` restores every `tasks.md` that existed at the pre-reset HEAD and is missing at the new HEAD (`git checkout <pre_reset_sha> -- <path>`, after clean so `-fd` cannot delete it) instead of raising because pre-RED was computed too early; the agent's pre-reset HEAD is captured on the per-attempt recovery ref `tmp/deviate-agent-work/<sanitized-task-id>/attempt-<rollback_attempts>` so a parent SIGTERM between `git reset` and `git clean` doesn't strand the discarded commit. After the reset, `_commit_judge_feedback_and_advance` appends a `tasks.jsonl` row (`judge_action=revert_red`, `judge_feedback`, status `PENDING`) and the `tasks.md` Judge Feedback bullet, then commits both paths in one `docs(<tid>): add judge feedback for retry` commit (`git add` those files only; not `session.json`). `session.red_commit_sha` is cleared (the boundary was discarded) and `pending_judge_action = "revert_red"`. `force_transition_to("RED")` so the task retries from scratch. `_escalate_to_new_red` consumes `revert_red` only after a RED-phase `red_commit_sha` lands. While that SHA is empty, the next `INVOKE_AGENT` stays RED (or the loop raises `TRAIN_EXHAUSTED` / `PhaseFailedError`). **No implicit fallback**: if `_resolve_pre_red_sha` returns empty AND `session.red_commit_sha` is empty, the runner raises `PhaseFailedError("ROLLBACK_BOUNDARY_MISSING ...")` rather than fall back to `HEAD~1`. When `pre_red` is unresolvable but `session.red_commit_sha` is known, that cached SHA is used as the explicit boundary.
           **Runner-level override on `failure_kind=test_defect`:** when `session.failure_kind == "test_defect"` and JUDGE emits `COMPLIANCE_VIOLATION`, `_coerce_judge_action` (`src/deviate/cli/micro.py`) forces `next_action="revert_red"` regardless of what the JUDGE manifest declared or omitted. The override reflects a contract invariant: when the RED test itself is wrong, looping back to GREEN with the same test is futile. After GREEN PASS (`failure_kind` empty / not `mechanical`), a `COMPLIANCE_VIOLATION` with structured Test Integrity (`violations[].category` matching Test Integrity / `Test Integrity Violation`, and/or `evaluation.test_integrity: FAIL`) uses optional `HandoverManifest.red_baseline_integrity: PASS | FAIL | null`. `PASS` retains original RED tests (`revert_green`); `FAIL` discards RED+GREEN (`revert_red`) and itself signals failed integrity. Null or omission preserves legacy routing; invalid values fail manifest validation. JUDGE verifies tests at `session.red_commit_sha`; unverified baselines use null. GREEN tampering still requires `evaluation.test_integrity: FAIL`. Verdict records include the classification. Feedback addresses the next phase. Honest-test implementation/scope gaps stay `revert_green`. Mechanical overlay keeps the agent's three-way choice. The runner parses `train_feedback` to detect a refactor-only `REFACTOR NOTE:` (clean pass after a green suite); structured Test Integrity still rejects, using the RED-baseline classification for rollback scope. `_run_tdd_cycle` honours the resulting `pending_judge_action == "revert_red"` by escalating now. It resets `green_attempts` to 0, increments `red_attempts`, and persists both on `.deviate/session.json`. It then dispatches `_run_red_phase(task, ..., bypass_phase_done=True)` so a fresh RED record appends to the append-only ledger (never rewriting prior entries). `_judge_feedback_from_manifest` normalizes structured `violations` into the same payload `JUDGE_REJECTED` already logs when `train_feedback` is absent; `_commit_judge_feedback_and_advance` persists that text as `**Judge Feedback**` after the `revert_red` reset. The retry RED prompt receives that same payload. `_inject_escalate_note` must not replace it with `previous cycle failed because test defect` when the payload is non-empty — the generic note is the fallback only when the payload is empty or is a GREEN pytest dump. When `_escalate_to_new_red` runs after `_apply_judge_verdict` already reverted and HEAD is the feedback commit, escalate does not reset again (a second pre-RED rollback would drop the persist). `TRAIN_EXHAUSTED` prints only after three RED escalates. The override is silenced on `COMPLIANCE_PASS` verdicts; JUDGE's outcome remains authoritative when the implementation is sound. The override preserves the legacy resolution for `revert_green`, `continue_refactor`, `proceed_to_refactor_no_diff`, and `skip_refactor` except the test_defect / no_failing_test / GREEN-PASS Test Integrity cases.
        2. **`revert_green`** — discard GREEN, preserve RED. `_run_judge_phase` threads `session.red_commit_sha` into `_execute_rollback`. When that SHA is empty, the runner raises `PhaseFailedError("ROLLBACK_BOUNDARY_MISSING ...")`. It does not print `ROLLBACK_FAILED … proceeding with train feedback`. It does not call `_commit_judge_feedback_and_advance`. It does not `force_transition_to("GREEN")`. When a RED-phase SHA exists, rollback still trains GREEN unless migration recovery failed: `ROLLBACK_RECOVERY_HOOK_MISSING` / `ROLLBACK_RECOVERY_HOOK_FAILED` is `ENV_NOT_READY` (GH-240) and does not print `ROLLBACK_FAILED … proceeding with train feedback`, does not call `_commit_judge_feedback_and_advance`, and does not `force_transition_to("GREEN")` or enter REFACTOR. After a successful reset, `_commit_judge_feedback_and_advance` appends a `tasks.jsonl` row (`judge_action=revert_green`, status `RED`) plus `tasks.md` Judge Feedback and commits those paths together. It advances `red_commit_sha` onto the docs-feedback commit only when the pre-call SHA is already a RED-phase failing-test commit and the new HEAD itself rests on a RED-phase ancestor (GH-236: a non-fatal `ROLLBACK_FAILED` retry that committed feedback on GREEN keeps the prior RED SHA). That advanced SHA remains the GREEN-entry / rollback boundary; the injected JUDGE diff base stays at the walked-back RED-phase commit (`_resolve_judge_diff_base`). GREEN-resume (`start_phase=JUDGE` after ledger GREEN) must honor `revert_green` as TRAIN GREEN: fall through into the GREEN train loop, or skip a second JUDGE when the session already holds that pending action plus `train_feedback`. Never enter REFACTOR or COMPLETED. `_finish_tdd_cycle` refuses both while pending is `revert_green` or `revert_red`. The in-loop TRAIN GREEN path and `revert_red` escalate-to-RED are unchanged.
    ...
    * **Retry state-drift guard:** First-pass zero-diff GREEN continues to JUDGE for empty-GREEN `test_quote` classification. A feedback-driven retry is different: if the task ledger already contains GREEN, `session.train_feedback` is populated, and the retry creates no implementation commit, `_run_green_phase()` raises `GREEN_STATE_DRIFT` instead of sending JUDGE a feedback-only diff. Recovery is explicit verification of the existing implementation followed by append-only ledger reconciliation. A leftover `REFACTOR NOTE:` after a clean `COMPLIANCE_PASS` is not a feedback-driven retry (GH-260): `_train_feedback_is_advisory` / `train_feedback_is_advisory` treat it as REFACTOR advice. `_run_green_phase` skips when GREEN is already done and the leftover is advisory; the TDD train loop does not treat that note as `session.train_feedback` retrain (does not burn GREEN/RED budget through TRAIN_EXHAUSTED). Spec-gap text, GREEN test dumps, and `judge_rejected` still train.
        After `revert_green`, GREEN-budget escalation commits the latest JUDGE feedback again after the pre-RED reset. It updates `tasks.md` and appends a `tasks.jsonl` row with `judge_action=revert_red` and status `PENDING`. It clears the RED boundary before committing feedback, then dispatches retry RED.

        `green_attempts` counts each GREEN phase run at start (including the first; max 3). TRAIN 3/3 is a real GREEN. After three GREEN runs the next train escalates (new RED, `green_budget_exhausted`) instead of raising `TRAIN_EXHAUSTED` or burning the last slot on escalate. `_run_tdd_cycle_impl` calls `_reset_tdd_retry_budget` once at cycle entry so a leftover TRAIN 3/3 in gitignored `.deviate/session.json` does not block a fresh `deviate micro run` (or the next `--all` task). That reset zeros only `green_attempts` / `red_attempts`; `train_feedback`, `red_commit_sha`, and `pending_judge_action` stay. Increment still happens at GREEN start in the same invocation. `revert_red` escalates immediately. After three RED escalates (`red_attempts >= 3`) the loop prints `TRAIN_EXHAUSTED`, raises `PhaseFailedError`, and marks the task `FAILED`. Before that budget is spent, `_apply_judge_verdict` compares the new rejection's Requirement/Correction text with the latest prior reject feedback (`verdicts.jsonl`, `tasks.md` Judge Feedback, `session.train_feedback`). A deterministic polar flip (strict identity matching vs preserve conflicting fixtures/tests about the same shared identity token), a unit↔integration layer flip, or A-B-A oscillation with conflicting requested vs provider values for the same returned identity logs `JUDGE_REQUIREMENT_CONTRADICTION`, marks the task `HITL_PENDING`, raises `HitlEscalationError`, and does not increment GREEN/RED train counters or roll back (GH-230, GH-253). Unit↔integration layer oscillation on a stamped task is prevented earlier by `<layer_lock>` (GH-248); this HITL remains the backstop. A-B-A token similarity alone is not a contradiction: additive restatements (changelog + verification, coverage + evidence) still train. Same-blast restated requirements still train. Same-blast `LOOP_DETECTED` telemetry (`streak >= 2`) is unchanged and does not halt. The runner honors the manifest verbatim — no interactive prompt. The legacy single-commit fallback (`verdict == COMPLIANCE_VIOLATION` without `next_action`) maps to `revert_green`.
...
- The **Green → Judge → Green loop** is the JUDGE → GREEN arrow at the top of
  ...
  `revert_green` is fatal (`DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` with
  planned `head_sha` / `recovery_ref`; never `HEAD~1`) and does not train
  GREEN. GREEN entry always persists a recoverable SHA then raises
  `GREEN_ENTRY_REFUSED` when it is still empty, including when
  `current_phase` is already GREEN. A present SHA that is not a
  RED-phase boundary is recovered first; if recovery fails the runner
  raises `DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` (GH-236) instead of
  `GREEN_ENTRY_REFUSED` that prints the leftover SHA. After a
  non-fatal `ROLLBACK_FAILED` TRAIN, `_maybe_advance_red_sha_past_feedback`
  advances onto the feedback commit only when that commit itself rests
  on a RED-phase ancestor. A migration-bearing rollback that cannot
  run the `[rollback]` / `test:reset` recovery hook is `ENV_NOT_READY`
  (`ROLLBACK_RECOVERY_HOOK_MISSING` / `ROLLBACK_RECOVERY_HOOK_FAILED`,
  GH-240) and does not train GREEN or enter REFACTOR. The `deviatdd`
  skill classifies a missing RED boundary as `DEVIATDD_BUG` (not only
  `BLOCKED`) and recommends `/deviate-green` after preserving evidence.
  It classifies a missing recovery hook as `ENV_NOT_READY` and
  recommends configuring the hook then restoring the isolated database.
  `green_attempts` trains GREEN up to 3 times then escalates; `TRAIN_EXHAUSTED`
  prints after three RED escalates (`red_attempts`). `_reset_tdd_retry_budget`
  zeros both counters at each TDD cycle entry so a leftover session budget
  cannot skip GREEN.
...
* **`/deviate-plan` Context (via `deviate plan pre`):** Spec-enriched issue file (intent + outlines) + Current Codebase State + workstation file paths parsed from the `## System Topology Mapping` section of the issue. Interactive TTY `deviate plan pre` also prints a soft 12-point handover checklist (`src/deviate/prompts/handover.py`) at Plan entry — banner plus questions, no prompt, no Gate 2. Off-TTY / `--json` / skill auto / `meso run` skip or log one line. Auto and manual Plan overlays inject the same shared text.
    * *System Directives:* Perform fresh localized research for this specific issue. Read the issue file, scan current codebase state (what exists now, not at epic-explore time), analyze what prior issues implemented via the `specs/issues.jsonl` ledger. Identify integration points, dependencies, potential conflicts. Produce `plan.md` containing the authoritative `## Acceptance Contract` section: every `AO-NNN` from the issue must be reconciled into one or more complete `AC-PLAN-NNN` Given/When/Then scenarios with `Source Outline`, `Upstream Traceability`, `Current-Code Evidence`, and `Verification Mode` blocks (each scenario carries exactly one `**Verification Mode**: automated|manual|deferred` line). Contextualize the issue for downstream task decomposition.
      ...
      * **Required sections in canonical order:** `## Plan Summary` → `## Acceptance Contract` → `## Workstation Mapping` → `## Risk Assessment` → `## Security Profile`. `## Data Flow Analysis` appears only when the issue moves or stores data; integration points fold into Workstation Mapping as `Integrates:` lines; constitutional compliance is a single `Constitution: §N` line. `AC-PLAN-NNN` identifiers MUST be sequential, zero-padded, and unique. The validator lives at `src/deviate/core/validation.py::validate_acceptance_contract` and the full schema block at `src/deviate/prompts/commands/deviate-plan.md` (mirrored in `src/deviate/prompts/auto/plan.md` for the runtime template).
      * **Grandfather registry:** shipped pre-alignment sample artifacts under `specs/` predate the aligned validators and are exempt from revalidation; unmarked legacy failures still fail.
* **`/deviate-tasks` Context (via `deviate tasks pre`):** Spec-enriched issue file (stories, scope, constraints) + authoritative `plan.md` Acceptance Contract + Codebase Layout Map + constitution command output.
    * *System Directives:* Tasks consumes two sources: the issue for intent and scope, `plan.md` for finalized Gherkin. Legacy issue/spec Gherkin is non-authoritative — the Tasks prompt halts with `PLAN_ACCEPTANCE_CONTRACT_MISSING`/`PLAN_ACCEPTANCE_CONTRACT_INVALID` when `plan.md` is absent or malformed, refusing to fall back to the issue's own Gherkin. Decompose the contract into discrete task entries written to `tasks.md` (the human-authored decomposition document). The CLI subsequently registers these as rows in `tasks.jsonl` (the append-only event ledger). Each task must include implementation hints (file locations, mock boundaries, fixture requirements) alongside the decomposition. Every entry must be assigned a unique tracking identifier (`TSK-{issue_suffix}-{NN}`) and must map cleanly to an `AC-PLAN-NNN` scenario in the plan contract. Encode DAG dependencies via `blocked_by` arrays in each task entry. Assign each task an execution type: `tdd` (standard TDD loop), `direct` (boilerplate/config, no RED phase), or `e2e` (end-to-end integration). Type `Verification_Batch` is locked to `IMMEDIATE` (never TDD) — the planner decision tree and `resolve_execution_mode` both enforce this; other types still pick TDD vs IMMEDIATE. Stamp every TDD task **Test Strategy** `unit` | `integration` | `e2e` (default unit; migration / live-DB AC → integration). Verification collects that layer plus cheaper existing rungs. Behavior-preservation decomposition splits independently invoked runtime hubs and rejects GREEN scopes that combine unrelated application, provider, and scheduler surfaces. Each TDD unit names one focused verification file with exact assertions for new behavior and any existing routes, middleware, jobs, exports, or handlers it touches. Import-safety and factory-behavior checks stay separate. Moving construction must preserve complete runtime configuration. Closing sweep only when a rung exists: user-facing + e2e → `[E2E]` Verification_Batch with the full existing ladder; else if integ exists → `[VERIFY]` unit-if-exists + integ; else no extra sweep. Never emit empty e2e files; never require integ. After Tasks commits `tasks.md`, `deviate run` chains directly into `deviate micro run --all` — no human-approval step.
...
* **Gate 1: Blueprint Approval (After `/deviate-research`, Before `/deviate-prd`)**
    ...
    * *Action:* Human reviews core architectural selections, design decisions, data models, and tech stacks. Reject schema-only `data-model.md` `## Data Flow` (no request-flow sequence / local-only marker, no outbound table). PRD and Shard execution remain locked until an approval flag is written.
...
retrained onto unit artifacts (GH-248). GH-230 / GH-253 contradiction HITL remains the backstop.
The agent must not infer the layer by reading
`tasks.md`. A TDD card that names more than one layer is rejected at `deviate tasks post`
with `MIXED_TEST_LAYER` (split into two TDD tasks). Micro dispatch fail-closes
with `SPLIT_TASK_REQUIRED` before any agent spawn when a mixed contract is
detected (GH-252); the token is preserved and is not remapped to
`TASK_NOT_FOUND`. `deviate red refresh-layer --task` restamps a stale ledger
layer from the operator-corrected card. Injected `test_strategy` /
`test_write_dir` / `test_command` stay one layer. IMMEDIATE `[VERIFY]` / `[E2E]` closing sweeps may still name
both layers. Classify from the
...
`ENV_NOT_READY`. Unit skips reset. Do not run `mise setup` or bare `mise reset`.
Without mise, map layers to conventional `tests/unit` /
...
1. **The Git Isolation Principle:** Every isolated task loop must be executed on a clean git branch or worktree environment. Commits are made automatically at each phase boundary via `_commit_phase()` in `micro.py` (`test: [{scope}]: RED phase`, `feat: [{scope}]: GREEN phase`, `refactor({scope}): REFACTOR phase`). COMPLETED `tasks.jsonl` rows are committed at the COMPLETE boundary: REFACTOR already includes the COMPLETED row in its polish commit; `skip_refactor`, EXECUTE JUDGE pass / `JUDGE_SKIP`, and Verification_Batch checkpoint persist the ledger via `_commit_completed_ledger` (ledger path only) when no polish commit exists (GH-231). `deviate micro run --review` (and `--all --review`) inserts an optional HITL pause via `_maybe_review_pause` immediately in front of `_commit_phase` / `_commit_phase_with_recovery` (`git add -A`) on RED, GREEN, REFACTOR, and EXECUTE: the runner prints `REVIEW_PAUSE <phase> <task_id>`, leaves the worktree dirty, and waits for TTY confirmation (`Enter` / yes) before committing. JUDGE feedback commits are not paused. Non-TTY / `--json` / missing stdin fail closed with `REVIEW_REQUIRES_TTY`. Off by default; no config key. This is not Gate 1 or Gate 3 and does not restore Gate 2. Worktrees are created via `deviate specify pre` using `create_worktree()` and removed via `remove_worktree()`. New worktree paths are rooted by `resolve_worktree_root(repo)` (`src/deviate/core/worktree.py`): `wt/` by default; sticky `.worktrees/` when that directory already exists and `wt/` does not; when both exist, new trees go under `wt/`. Existing trees are operated wherever `git worktree list` already has them (`find_worktree_for_branch`). There is no auto-migrate/`mv` of `.worktrees` → `wt` (git stores absolute worktree paths). When creating a worktree, after `git worktree add` the orchestrator copies agent skill directories (`.claude/`, `.opencode/`, `.factory/`, `.pi/`, `.omp/`) and `.env` (if present) from the repo root into the worktree so that skills and local configuration are available without re-running `deviate setup`.

2. **The Scope Audit Law:** When entering or running the `GREEN` execution phase, the system checks for unauthorized changes to test, spec, and config directories. Dependency manifests and lockfiles are permitted when the task explicitly changes dependencies, including `pyproject.toml` and `uv.lock`; JUDGE verifies those changes are task-related and consistent. Protected files are reverted via `git restore <filepath>`. The JUDGE phase (`deviate judge pre`) additionally performs compliance verification by detecting changes to protected modules declared in `spec.md` `Module:` lines. Complements the GREEN stub-PASS guard in `_run_green_phase` (see `DeviaTDD-api.md` § GREEN Stub-PASS Guard): scope rejects writes the agent shouldn't have made; the stub-PASS guard rejects passes the agent shouldn't have emitted.
...
8. **The Session Continuity Principle:** Session state is persisted to `.deviate/session.json` after each CLI command. The `SessionState` class tracks `current_phase`, `active_issue_id`, and `last_command`. Macro and meso phases transition through `transition_to()` with validation from `_MACRO_TRANSITION_MAP`. Micro phases use `force_transition_to()`. `_run_single()` resumes from the latest `tasks.jsonl` status via `start_phase` (`_start_phase_from_status`). `TASK_ALREADY_DONE` is only for ledger `COMPLETED`. Ledger `JUDGE` / `REFACTOR` / `YELLOW` with session `IDLE` is mid-flight and re-enters that phase — the session cache is not treated as a terminal skip. Model continuity and KV cache management are delegated to the calling environment.
...
| `pi` | `pi -p` | Commands file-copied into `<workdir>/.pi/prompts/<name>.md` (project-local; flat top-level only per Pi's documented slash-command convention) | `--model <id>` flag (accepts `provider/model` shorthand) | Lean spawn after `pi -p`: `--tools read,bash,edit,write`, `--no-skills`, optional `--skill` (no `--no-extensions`, so extension-registered providers load); schema-limit tokens abort as `AGENT_ERROR` |
...
3. **Model flag injection.** Pi print mode (`pi -p`) accepts the
   ...
   to `opencode` / `droid` behavior. (`claude` uses print mode but ignores `--model`.)
4. **Print mode only.** Pi runs single-shot print mode (`pi -p`, exits after the first assistant turn).
   No RPC mode remains.
5. **Lean tool policy.** After the print-mode prefix (`pi -p`), `invoke` appends
   ...
   `BACKEND_COMMANDS["pi"]` stays `pi -p`.
...
Plan is ready, fails loudly on a plan whose acceptance scenarios lack the
`**Verification Mode**:` line (no default is inserted), and
...
skill dirs. `wt/`, `.worktrees/`, `.deviate/`, and `.zvec-grep/` exclude local
