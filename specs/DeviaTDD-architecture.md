...
- The **Green → Judge → Green loop** is the JUDGE → GREEN arrow at the top of
  ...
  `GREEN_ENTRY_REFUSED` that prints the leftover SHA. After
  `ROLLBACK_FAILED` TRAIN, `_maybe_advance_red_sha_past_feedback`
  advances onto the feedback commit only when that commit itself rests
  on a RED-phase ancestor. The `deviatdd` skill classifies that
  failure as `DEVIATDD_BUG` (not only `BLOCKED`) and recommends
  `/deviate-green` after preserving evidence.
