...
- The **Green → Judge → Green loop** is the JUDGE → GREEN arrow at the top of
  ...
  GREEN with `<train_feedback>` injected. `green_attempts` trains GREEN
  up to 3 times then escalates; `TRAIN_EXHAUSTED` prints after three
  RED escalates (`red_attempts`).
