...
the command's exit code plus `_is_no_tests_collected` (pytest exit 5). Exit 127 /
command-not-found / a missing mise binary is `ENV_NOT_READY` via
`_is_verification_env_not_ready` — never `_adjudicate_red_no_failing_test`.
`_run_test_cmd` / `_execute_test_command` / `run_safe_command` prepend mise
shims and `mise bin-paths` onto the subprocess PATH (and wrap a bare tool as
`mise exec --` when the binary is missing but `mise` exists) so login-shell
toolchains resolve under a GUI/Cursor PATH. `_run_pytest()` remains the Python-specific subprocess runner
...
| **Elixir / ExUnit** | `mix test` (via `_MANIFEST_TEST_COMMANDS` / `mise exec`) | `returncode == 0` | exit code + `_is_no_tests_collected`; 127 is `ENV_NOT_READY` | Same. |
| **Rust / cargo** | `cargo test` (via `_MANIFEST_TEST_COMMANDS`) | `returncode == 0` | exit code + `_is_no_tests_collected`; 127 is `ENV_NOT_READY` | Same. |
| **Go / testing** | `go test ./...` (via `_MANIFEST_TEST_COMMANDS`) | `returncode == 0` | exit code + `_is_no_tests_collected`; 127 is `ENV_NOT_READY` | Same. |
| **Node.js / Jest** | `npm test` (via `_MANIFEST_TEST_COMMANDS`) | `returncode == 0` | exit code + `_is_no_tests_collected`; 127 is `ENV_NOT_READY` | Same. |
