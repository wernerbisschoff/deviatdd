...
- TSK-017-01: Add `DeviateConfig.claim_remote` and `resolve_claim_remote`
  ...
  - **Verification**: `pytest tests/unit/test_state/test_config.py -k claim_remote -v`
...
- TSK-017-02: Resolve effective local in `_specify_pre` and skip the remote lock
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_cli/test_meso.py::TestSpecifyLocalFlag tests/unit/test_meso/test_specify.py -k "local or claim_remote" -v`
  ...
  - **Details**:
    - **Red**: In `TestSpecifyLocalFlag`, add a config-false case that writes `.deviate/config.toml` with `claim_remote = false`, calls `_specify_pre` with `local=False`, and asserts `branch_exists_on_remote` and `git push` are never called while the worktree is created and SPECIFIED is written. Keep the four existing `local=True` pins. In `tests/unit/test_meso/test_specify.py`, add `test_specify_omitted_local_honors_claim_remote_false` (omitted `--local` forwards effective `local=True`) and `test_specify_local_overrides_claim_remote_true` (`--local` still forwards `True` when config is true). Assert `LOCAL_ONLY` on the skip-push path. Use `tmp_git_repo` + `_git_env()`.
...
- TSK-017-03: Skip origin-as-claimed in local discovery
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_meso/test_meso_orchestration.py::TestDiscoverClaimableIssue -v`
  - **Estimated Time**: 45 minutes
  - **Flow References**: []
  - **Files**:
    - `src/deviate/cli/meso.py`
...
- TSK-017-04: Forward `--local` on `meso run` and `deviate run`; keep `--no-setup` separate
  ...
  - **Verification**: `pytest tests/unit/test_meso/test_meso_orchestration.py::TestMesoRunNoSetup tests/unit/test_cli/test_top_level_run.py -k "local or no_setup or meso_run" -v`
  - **Estimated Time**: 75 minutes
  - **Flow References**: []
  - **Files**:
    - `src/deviate/cli/meso.py`
    ...
    - `tests/unit/test_meso/test_meso_orchestration.py`
    - `tests/unit/test_cli/test_top_level_run.py`
...
- TSK-017-05: Persist `claim_remote` via `deviate setup --no-claim-remote`
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_cli/test_init.py -k claim_remote -v`
...
- `tests/unit/test_meso/test_meso_orchestration.py` touched by TSK-017-03 and TSK-017-04
