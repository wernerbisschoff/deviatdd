...
- TSK-015-01: explore_post commits flows.jsonl atomically with explore.md
  ...
  - **Verification**: `mise run test tests/test_macro/test_explore.py::test_explore_post_commits_flows_ledger_atomically -v`
...
- TSK-015-02: Create `deviate release tag-included` CLI subcommand
  ...
  - **Details**:
    - **Red**: Write three tests in `tests/test_cli/test_release.py`:
...
- TSK-015-03: Register `release_app` in the root CLI
  ...
  - **Details**:
    - **Red**: Write `test_cli_help_lists_release_group` in `tests/test_cli/test_help.py` — invoke `deviate --help`, assert `release` appears in the command list with the description from the Typer decorator.
...
- TSK-015-06: Remove retired event types from `FlowEvent.event_type` Literal union
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `mise run test tests/test_state/test_ledger.py -v`
  - **Estimated Time**: 30 minutes
  - **Files**:
    - `src/deviate/state/ledger.py`
    - `tests/test_state/test_ledger.py`
    - `tests/test_cli/test_inspect.py`
    - `tests/test_core/test_flow_confirmation.py`
...
- TSK-015-07: Simplify `_derive_impl_status` to two-valued Literal
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `mise run test tests/test_state/test_ledger.py::test_derive_impl_status -v`
...
- TSK-015-09: Drop tests for retired event types
  ...
  - **Details**:
    - **Green**: Drop `tests/test_cli/test_inspect.py:812` and `:871` (test cases that write `FLOW_INCLUDED_IN_RELEASE` events — keep these, only drop the retired types). Drop `tests/test_core/test_flow_confirmation.py:454` and `:490` (test cases for retired types).
