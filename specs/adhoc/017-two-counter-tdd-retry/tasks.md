...
- TSK-017-02: Train GREEN three times on `revert_to_red`, then escalate
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `uv run pytest tests/test_micro/test_two_counter_retry.py -k revert_to_red -q`
...
- TSK-017-03: Escalate on `revert_before` and stop after three escalates
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `uv run pytest tests/test_cli/test_micro.py::TestRunnerLoopRestartsRedOnRevertBefore tests/test_micro/test_two_counter_retry.py -k revert_before -q`
  - **Estimated Time**: 90 minutes
  - **Flow References**: []
  - **Files**:
    - `src/deviate/cli/micro.py`
...
- Files touched by multiple phases: `src/deviate/cli/micro.py`, `tests/test_micro/test_two_counter_retry.py`
