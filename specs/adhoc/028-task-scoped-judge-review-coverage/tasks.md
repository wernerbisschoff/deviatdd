...
- TSK-028-02: Gate TDD JUDGE on resolved task tokens and drop cite-every-plan wording
  ...
  - **Verification**: `uv run pytest tests/unit/test_micro/test_judge.py -q --tb=short`
...
- TSK-028-03: Fail-close review when a plan AC has no COMPLETED claim
  ...
  - **Mode**: IMMEDIATE
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `uv run pytest tests/unit/test_cli/test_review.py -q --tb=short`
  ...
  - **Rationale**: US-028-03 and `AC-PLAN-005` require `deviate review pre` and `deviate review post` to exit non-zero or emit a non-PASS contract when any this-issue `plan.md` token lacks a COMPLETED claim. `AC-PLAN-006` requires READY or PASS when every token is claimed by a this-issue COMPLETED row or by persisted evidence already on that COMPLETED raw JSONL object. `AC-PLAN-007` requires vacuous complete when `plan.md` or tokens are absent, and ignores PENDING, FAILED, and sibling-issue rows. `src/deviate/core/review_coverage.py` is the unit-testable scan. `src/deviate/cli/review.py` owns `pre` and `post`. `tests/unit/test_cli/test_review.py` owns `test_review_pre_emits_contract`. Constitution §1 HITL Gate 3: review owns issue-level completeness. Constitution §1 Append-Only Ledger: parse latest `tasks.jsonl` rows; add no #84 field. Constitution §3: `tmp_git_repo` plus `_git_env()`.
  - **Details**:
...
- TSK-028-04: Document task-scoped JUDGE evidence and review coverage
  - **Type**: Config
  - **Mode**: IMMEDIATE
  - **Verification**: `uv run pytest tests/unit/test_core/test_judge_evidence.py tests/unit/test_micro/test_judge.py tests/unit/test_cli/test_review.py -q --tb=short`
