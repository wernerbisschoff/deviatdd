...
- TSK-026-01: Append lean Pi flags and keep the four coding tools
  ...
  - **Verification**: `uv run pytest tests/unit/test_core/test_agent.py tests/unit/core/test_agent.py -q -k "pi and (rpc or tool or lean or skill or invoke)"`
...
### Tasks
  ...
  - **Judge Feedback**: JUDGE evidence test_path is not in the injected diff or HEAD: tests/unit/test_core/test_agent.py
- TSK-026-02: Abort the child on the first schema-rejection line
  - **Type**: Bugfix
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `uv run pytest tests/unit/test_core/test_agent.py tests/unit/core/test_agent.py -q -k "schema or tool_count or unsupported_tool"`
  - **Estimated Time**: 60 minutes
  - **Flow References**: []
  - **Files**:
    - `src/deviate/core/agent.py`
...
- TSK-026-04: Document lean Pi spawn and token-bearing `AGENT_ERROR`
  - **Type**: Config
  - **Mode**: IMMEDIATE
  - **Verification**: `uv run pytest tests/unit/test_core/test_agent.py tests/unit/core/test_agent.py tests/unit/test_cli/test_micro.py -q -k "pi and (rpc or tool or lean or schema or invoke or skill or AGENT_ERROR)"`
...
- Files touched by multiple phases: `src/deviate/core/agent.py`, `tests/unit/test_core/test_agent.py`, `tests/unit/core/test_agent.py`. Phase 1 owns lean argv. Phase 2 owns schema-token abort. Phase 3 owns `src/deviate/cli/micro.py`. Phase 4 owns specs and CHANGELOG only.
