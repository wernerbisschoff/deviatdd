...
- TSK-009-01: Register Pi backend in config model, command registry, model-flag dispatch, error handling, and test fixtures
  ...
  - **Verification**: `mise run test tests/unit/core/test_agent.py -v`
  ...
  - **Details**:
    ...
    - **Acceptance**: `mise run test tests/unit/core/test_agent.py -v` exits 0. All tests: literal accept, literal reject, subprocess contract, model flag not injected, missing binary, YAML extraction, stub backend — pass. `AgentConfig(backend="pi")` round-trips through `model_dump()`/`model_validate()`. `BACKEND_COMMANDS["pi"]` resolves to `"pi -p"`.
...
- TSK-009-02: Add Pi to agent selection constants and implement `deviate setup` Pi-as-project-local flow: file-copy skills into `<workdir>/.pi/skills/`, no `settings.json` generation, no `~/.pi/agent/` writes
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `mise run test tests/unit/test_cli/test_init.py -v`
...
- TSK-009-03: Implement RPC mode dispatch when `pi_rpc = true` — spawn `pi --mode rpc --no-session`, send JSONL prompt, parse stream, extract manifest from `agent_end`
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `mise run test tests/unit/core/test_agent.py::test_pi_rpc_mode_opt_in -v`
  - **Estimated Time**: 60 minutes
  - **Files**:
    - `src/deviate/core/agent.py`
    - `tests/unit/core/test_agent.py`
...
- TSK-009-04: Extract `pi.session_stats` from Pi agent output and enrich prompts.log AGENT_RESULT entries
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `mise run test tests/unit/core/test_agent.py::test_pi_session_stats_logged -v`
  - **Estimated Time**: 60 minutes
  - **Files**:
...
- `tests/unit/core/test_agent.py`: touched by TSK-009-01, TSK-009-03, TSK-009-04
