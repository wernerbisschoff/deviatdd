...
- TSK-022-01: Reject empty declared files on TDD already_satisfied
  ...
  - **Verification**: `uv run pytest tests/test_micro/test_orchestration.py tests/test_micro/test_judge.py -q -k "already_satisfied or no_failing_test or execute_judge_stays_ungated" --tb=short`
...
- TSK-022-03: Align API, architecture, and CHANGELOG with the files gate
  ...
  - **Files**:
    ...
    - `CHANGELOG.md`
  - **Rationale**: `AC-PLAN-007` plus constitution §5 Definition of Done require `specs/DeviaTDD-api.md`, `specs/DeviaTDD-architecture.md`, and `CHANGELOG.md` `[Unreleased]` in the same change as the runner contract. US-022-01 and US-022-02 are the user-visible COMPLETE rule: named present regression tests. AGENTS.md Spec Alignment requires both spec files. ISS-ADH-020 fixtures in `tests/test_micro/test_judge.py` and ISS-ADH-021 SHA pins in `tests/test_micro/test_green.py` and `tests/test_micro/test_two_counter_retry.py` must stay green. Constitution §1 Four-Layer Architecture: this slice does not skip a layer and does not restore Gate 2.
  - **Details**:
