...
- [x] T002: SessionState Meso Phase Transition Map
  ...
  - **Details**:
    ...
    - **Acceptance**: All transition tests pass. Existing macro tests (`tests/unit/test_macro/`) continue to pass unchanged. SPECIFY/TASKS/IDLE transitions behave correctly.
...
- [x] T003: `deviate specify` — Issue Record Resolution & Artifact Scaffolding
  ...
  - **Verification**: `pytest tests/unit/test_meso/test_specify.py -v`
...
- [x] T004: `deviate tasks` — TaskRecord Generation, Append & Idempotency Guard
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_meso/test_tasks.py -v`
  - **Estimated Time**: 75 minutes
  - **Dependency**: T001
  - **Files**:
    - `src/deviate/cli/meso.py`
    - `tests/unit/test_meso/test_tasks.py`
...
**Risk Hotspots**:
- `src/deviate/state/config.py` — `_TRANSITION_MAP` refactoring from flat `dict[str, str]` to branching `dict[str, tuple[str, ...]]` must preserve backward compatibility with all existing macro-layer transitions. Existing macro tests in `tests/unit/test_macro/` must continue to pass.
...
- `tests/unit/test_state/test_ledger.py` — TaskRecord model (+6 tests)
- `tests/unit/test_state/test_config.py` — meso transitions (+4 tests)
- `tests/unit/test_meso/test_specify.py` — specify command (+5 tests)
- `tests/unit/test_meso/test_tasks.py` — tasks command (+5 tests)
