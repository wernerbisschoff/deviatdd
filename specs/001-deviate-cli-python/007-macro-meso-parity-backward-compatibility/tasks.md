...
- [x] T001: Enhance macro contract infrastructure and apply to explore/research pre
  ...
  - **Details**:
    ...
    - **Acceptance**: `pytest tests/unit/test_cli/test_macro_contracts.py -v` passes; manual `deviate explore pre "test" --dry-run` emits a contract with at least 15 fields matching bash output.
...
- [x] T002: Extend enhanced contract to prd and shard pre commands
  ...
  - **Details**:
    ...
    - **Acceptance**: `pytest tests/unit/test_cli/test_macro_contracts.py -v` passes; manual `deviate prd pre --dry-run` emits 17+ fields; `deviate shard pre --dry-run` emits 17+ fields.
...
- [x] T003: Implement full JSON contract emission for tasks pre and pr pre
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_cli/test_meso_contracts.py -v`
  ...
  - **Details**:
    ...
    - **Acceptance**: `pytest tests/unit/test_cli/test_meso_contracts.py -v` passes; `deviate tasks pre --dry-run` emits valid JSON with all 9 required fields; `deviate pr pre --dry-run` emits valid JSON with all 8 required fields.
...
- [x] T004: Implement content validation engine and wire into post-commands
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_core/test_validation.py -v`
  - **Estimated Time**: 90 minutes
  - **Files**:
  ...
  - **Details**:
    ...
    - **Acceptance**: `pytest tests/unit/test_core/test_validation.py -v` passes; `deviate explore post` on a minimal explore.md fails with specific missing section names; `deviate shard post` validates YAML frontmatter not spec.md/tasks.md.
...
- [x] T005: Add dry-run flag to remaining commands and issue-id to tasks post
  ...
  - **Mode**: TDD
  - **Test Strategy**: Sociable_Unit
  - **Verification**: `pytest tests/unit/test_cli/test_macro_contracts.py tests/unit/test_cli/test_meso_contracts.py -v`
  - **Estimated Time**: 60 minutes
  ...
  - **Details**:
    - **Red**: Write `test_prd_pre_dry_run_does_not_create_artifacts()` asserting that after `--dry-run`, no explore.md/design.md files exist in the expected bucket. Write `test_shard_pre_dry_run_does_not_create_issues()` asserting the issues ledger is unchanged. Write `test_tasks_pre_dry_run_does_not_append_ledger()` asserting no new task records appear. Write `test_tasks_post_issue_id_resolves_correct_spec()` asserting that `--issue-id ISS-001-006` validates the correct tasks.md.
    ...
    - **Acceptance**: `pytest tests/unit/test_cli/test_macro_contracts.py -v` and `tests/unit/test_cli/test_meso_contracts.py -v` pass; `deviate prd pre --dry-run` shows contract but no files created; `deviate tasks post --issue-id ISS-999` exits with `INVALID_ISSUE_ID`.
...
- [x] T007: Implement deviate run dispatcher with TDD/execute routing
  ...
  - **Details**:
    ...
    - **Acceptance**: `pytest tests/unit/test_micro/test_run.py -v` passes; `deviate run T001` dispatches correctly; `deviate run TSK-007-01` dispatches correctly; `deviate run --all` processes all tasks.
...
- [x] T008: End-to-end parity verification suite
  - **Type**: Infra_Batch
  - **Mode**: TDD
  - **Test Strategy**: Integration
  - **Verification**: `pytest tests/test_integration/test_parity.py -v`
...
- `tests/unit/test_cli/test_macro_contracts.py`: Touched by T001, T002, T005
