...
- Verified test surfaces: `tests/unit/test_cli/test_setup.py` (per-agent isolation, Codex Luna/reasoning upserts, no-clobber); `tests/unit/test_state/test_config.py` (`DeviateConfig.profile == "default"`, `use_libref` default False, unused `ProfileConfig`); `tests/unit/test_core/test_profile.py` (`resolve_profile` public names are `full` / `fast`; legacy `secure` is an internal alias); `tests/test_integration/test_skill_installation.py`.
...
- **Quality, Safety & Observability**: Tests use `typer.testing.CliRunner` invoking the `cli` group from a temp `chdir`. `tests/unit/test_cli/test_setup.py` pins per-agent isolation and Codex Luna/reasoning no-clobber. `tests/unit/test_state/test_config.py` asserts `config.profile == "default"`. `tests/unit/test_core/test_profile.py` asserts `resolve_profile("invalid")` raises and that public names are `full` / `fast`.
...
| `tests/unit/test_cli/test_setup.py` | Test | Pins per-agent isolation and Codex Luna / reasoning no-clobber. | `result = runner.invoke(cli, ["setup", "--agent", "opencode"])` `assert result.exit_code == 0, result.output` `assert (tmp_path / ".opencode" / "commands" / "deviate-red.md").is_file()` |
| `tests/unit/test_state/test_config.py` | Test | Pins `DeviateConfig.profile == "default"`. | `config = DeviateConfig()` `assert config.profile == "default"` `assert config.timeout_seconds == 1800` |
| `tests/unit/test_core/test_profile.py` | Test | Pins `resolve_profile` rejects unknown names; public set is full/fast. | `with pytest.raises(ValueError) as exc:` `    resolve_profile("invalid")` `msg = str(exc.value).lower()` `assert "full" in msg` |
