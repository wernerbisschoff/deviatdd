...
- Option C: Unify auto + manual into one agent-invoking orchestrator — rejected. Explore `## Architectural Baselines`: "The manual path has no agent invocation — the human's agent acts on the prompt; the CLI only supplies contract (pre) and applies side effects (post)." In-process invocation on the manual surface would change side-effect timing and break the `GH-154 AC-6` contract (`tests/test_micro/test_red.py`: "manual `deviate red post --task-id` matches the pending record").
...
| K2 — Keep two surface orchestrators; share only per-step kernels | Gain: auto semantics unchanged, manual side-effect timing preserved. Lose: two orchestration bodies remain (they are thin) | Explore File Registry: `tests/test_micro/test_red.py` — "GH-154 AC-6: manual `deviate red post --task-id` matches the pending record"; surface entry semantics are part of the installed contract |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| RSK-001 | Status-token drift between auto and manual surfaces after extraction (`RED_POST_OK`, `GREEN_POST_OK`, `JUDGE_POST_OK route=...`, `REFACTOR_POST_OK`, `TEST_NOT_FOUND`, `LEDGER_UPDATE_FAILED`) | M | H — prompt retry contracts reference the tokens | One `KernelOutcome.status_token` per outcome; regression tests assert each token on both surfaces | `src/deviate/cli/micro.py` | Explore `## Architectural Baselines` (Quality section) + `tests/test_meso/test_prompt_assembly.py:70` |
...
| RSK-003 | Test-suite slowdown: new tests reaching `_run_pytest` as a subprocess | M | M — AGENTS.md performance rule (full suite < 30s) | All kernel tests mock `deviate.cli.micro._run_pytest` with a `subprocess.CompletedProcess` fixture | `tests/test_micro/` | AGENTS.md "Test Performance" + explore Test Runner Configuration |
...
| RSK-006 | Git isolation breakage in tests (kernel tests calling real git) | L | M — pollutes the working repo | Kernel tests reuse `tests/conftest.py` `_git_env` + `tmp_git_repo`; every git call sets `cwd=<tmp_git_repo>` | `tests/test_micro/` | AGENTS.md "Git Isolation" + explore Test Runner Configuration |
...
| §3 "Test framework: pytest ... Test command: `pytest tests/ -v`" | New kernel tests land in `tests/test_micro/` as pytest tests | Aligned | Explore Test Runner Configuration: micro-layer tests in `tests/test_micro/` |
...
| SRC-007 | Codebase_File | `tests/test_micro/test_red.py` | GH-154 AC-6 manual `red post --task-id` contract |
| SRC-008 | Codebase_File | `tests/test_micro/test_judge.py` | Manual `judge post` applies auto-mode verdict side effects |
| SRC-009 | Codebase_File | `tests/test_meso/test_prompt_assembly.py` | Status-token / retry-contract coupling (`:70`) |
