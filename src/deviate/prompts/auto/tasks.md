...
9. **Adapter Transport Split**: When the slice names an external SDK or provider adapter, split adapter transport from port behavior into its own task card. Each adapter task card carries one concrete contract row per method: dependency signature, auth wiring, request identity preservation, response lookup.
