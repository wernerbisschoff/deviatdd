...
2. **Downstream Sharding Readiness**: Every FR has a stable AO-NNN behavioral outline. PRD MUST NOT emit Given/When/Then; final Gherkin acceptance criteria belong to Plan. FRs are traceability units only.
...
- All `FR-[ID]` and `AO-[ID]` tokens must be unique and sequential.
