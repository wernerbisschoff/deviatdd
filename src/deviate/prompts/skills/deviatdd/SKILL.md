...
| `JUDGE_REQUIREMENT_CONTRADICTION` / `HITL_REQUIRED` | Successive JUDGE requirements conflict (strict identity matching vs preserve a conflicting fixture, requested vs provider identity, or unit↔integration layer). Do not keep training. Choose one spec interpretation, correct the fixture or requirement, then retry. Additive restatements (changelog + verification, coverage + evidence) are not contradictions — keep bounded training. A unit↔integration layer flip on a stamped task should have been prevented by `<layer_lock>` (GH-248); this HITL is the backstop. |
...
| `JUDGE_REQUIREMENT_CONTRADICTION` / `HITL_PENDING` after oscillating JUDGE feedback | Stop only when the poles conflict (identity source or test layer). Additive restatements are not a spec choice — keep training until the budget exhausts. For a real conflict, choose one interpretation, correct the conflicting spec/test, commit that correction, then retry the task. | No training retry until the operator chooses a real conflict. |
...
| `ROLLBACK_RECOVERY_HOOK_MISSING` / `ENV_NOT_READY: ROLLBACK_RECOVERY_HOOK_MISSING` | Migration-bearing JUDGE rollback had no `[rollback]` hook and no `test:reset` mise task; runner stopped instead of printing `ROLLBACK_FAILED … proceeding with train feedback` | Environment not ready, not a task-level GREEN/REFACTOR failure. Do **not** retry GREEN or REFACTOR on the unrestored catalog. Configure `[rollback] command` in `.deviate/config.toml` or add `test:reset` via `/deviate-init`, restore the isolated database, then retry. Classify the skill output as `ENV_NOT_READY`. |
| `ROLLBACK_BOUNDARY_MISSING` / `DEVIATDD_BUG: ROLLBACK_BOUNDARY_MISSING` | JUDGE `revert_green` has no `session.red_commit_sha`, or GREEN entry holds a SHA that is not a RED-phase failing-test boundary (often after a failed rollback recovery); runner printed `head_sha` / `recovery_ref` / `red_commit_sha` | Harness bug, not only `BLOCKED`. Preserve the log and recovery details (`git show <head_sha>` / `git switch <recovery_ref>`). Classify the skill output as `DEVIATDD_BUG`. After preserving evidence, recommend `/deviate-green`. Do not retry with `HEAD~1`. Do not treat a GREEN-entry refusal that prints a leftover SHA as a missing-RED task failure. |
