...
1. **One idea per sentence.** Split compound sentences at each independent idea.
...
2. **One semantic per field, no synonyms.** Each labeled field, key, or ID carries exactly one meaning.
   Do not reuse one field for two concepts, do not split one concept across two fields, and do not
   introduce a second label for a concept that already has a canonical label. Reuse the existing label.
3. **Describe quantities and conditions explicitly** in field descriptions. Prefer
