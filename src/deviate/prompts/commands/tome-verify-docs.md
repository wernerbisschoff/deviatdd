...
- **IA components checked**:
  ...
  5. **`prev` / `next` chain integrity**: when a page's `prev` is set to a non-null path, the page at that path MUST carry `next: <this-page>`. When a page's `next` is set to a non-null path, the page at that path MUST carry `prev: <this-page>`. A page reachable only one direction is a broken chain.
  6. **Classifier reconciliation**: when a `/tome-classify` report is in context, the page's `prev` MUST match the classifier's `parent` field (or be `null` when the classifier's `parent` is `null`); the page's `next` MUST match the classifier's `next` field (or be `null` when the classifier's `next` is `null`).
