...
The injected test layer (`test_strategy` / `test_write_dir` / `test_command`) is locked. Do not require a different layer than the contract. GH-230 / GH-253 HITL remains the backstop when successive requirements still contradict; additive restatements are not contradictions.
