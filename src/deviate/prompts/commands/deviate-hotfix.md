...
<step_1_identify_targets>zvec_grep_search "function calc_total" → cart.py, test file → test_cart.py</step_1_identify_targets>
...
2. Identify broken file(s): use zvec-grep through the `zvec_grep_search` MCP tool or `zg query` via the CLI to locate files matching bug keywords; use `grep` / `find` for exact matches.
...
| NO_BUG_LOCATION | Use zvec-grep through the `zvec_grep_search` MCP tool or `zg query` via the CLI for semantic matches of relevant keywords, function names, or error messages mentioned in the bug report; use `grep` for literal keyword search. |
