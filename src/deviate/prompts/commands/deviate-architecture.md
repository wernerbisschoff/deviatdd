...
3. **Architecture Schema**: `specs/_product/architecture.md` MUST document:
   ...
   between components. A component MAY name the `FLOW-NN` IDs it relates
   to as a prose cross-reference. There is no machine-consumed mapping.
...
8. **Prose Cross-References Only**: Flow references in `architecture.md` are prose for human readers. No downstream phase derives machine state from them — there is no `flow_refs` contract.
...
## 8. Consistency Check
Cross-check prose references: a component that names a `FLOW-NN` ID SHOULD point at a flow that exists in the catalog, and vice versa. Surface gaps as `[yellow]TRACEABILITY_GAP[/]` warnings for the human to resolve. These are advisory only.

## 9. Next Step
Inform the user that `/deviate-release` may reference this architecture in prose. Do NOT trigger any downstream phase automatically.
...
| Component references no flow ID | Surface as `[yellow]ORPHAN_COMPONENT[/]` advisory note (prose-level only) |
| Flow ID references no component | Surface as `[yellow]ORPHAN_FLOW[/]` advisory note (prose-level only) |
