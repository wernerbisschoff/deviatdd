...
10. **Output Format**: Present the final response as a single fenced ```` ```text ```` block containing the setup log lines plus the readiness signal per `<readiness_signal>`. No preamble, no postamble, no XML wrapper.
