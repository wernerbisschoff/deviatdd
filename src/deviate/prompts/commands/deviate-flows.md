...
8. **Concise Output Discipline**:
   Flow blocks stay scannable in a single screen. The following length
   ...
   - `Trigger`: 1 sentence (or 1 short bullet) — the canonical entry
     point of the flow. Always present.
   ...
   - `Happy path (primary steps)`: 3–7 numbered steps, ≤ 12 words each.
     Always present — this is the testable core of the flow.
...
9. **Commit at Sign-Off (end-of-session atomic commit)**:
   ...
   **Phase A — Draft.** For each new flow, write the
   ...
   uncommitted so the user can review with `git diff`. Chat-only output
   is not enough; the files MUST land on disk.
...
| Generated flow block exceeds the line/word budgets in invariant 8 | Tighten prose before writing; keep each flow scannable in a single screen |
