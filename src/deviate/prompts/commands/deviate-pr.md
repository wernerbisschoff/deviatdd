...
CRITICAL INVARIANTS:
1. **Ledger Update Rule**: Append COMPLETED event after PR is successfully created (not only after merge). The event format: `{"issue_id":"<ISS_ID>","status":"COMPLETED","timestamp":"..."}` (`<ISS_ID>` is `<epic-prefix>-<ordinal>` for new work, e.g. `002-001`; legacy `ISS-NNN` is grandfathered).
...
- **issue_id**: e.g. `002-001` for new work, `ISS-001` for grandfathered ids
- **description**: the raw issue title with any bracketed prefix (e.g. `[FR-NNN]`) stripped

Examples:
- `feat(002-001): CLI Architecture Realignment & Skill Integration`
- `chore(002-002): close 002-002 and remove deviate-context skill`
- `fix(002-003): handle null pointer in user lookup`
