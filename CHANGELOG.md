...
### Added
- **Generated technical-document prose now follows ASD-STE100 Simplified Technical English.** Both prompt composers (`load_template()` in `src/deviate/prompts/assembly.py`, auto mode, and `compose_command_body()` in `src/deviate/core/commands.py`, manual slash-command mode) inject a shared `src/deviate/prompts/core/style-ste.md` directive before every phase body. It is self-scoping: **Track A** (one idea per sentence, active voice, present tense, ≤~20-word sentences, single qualifier per noun, approved/concrete vocabulary, explicit quantities, consistent terminology) applies only when the agent authors prose in documents (PRD, design, data-model, architecture, domain-model, flows, release notes, PR bodies, ADRs, review HTML); **Track B** (exact token preservation, one semantic per field, no synonym fields, explicit conditions/quantities) applies always, including to handover manifests and structured output. Neither track rewrites code, identifiers, file paths, command names, JSON/YAML keys, or quoted structure — structural constraints win on collision, mirroring STE100's controlled-vocabulary design. Specs (`specs/DeviaTDD-api.md` prompt tree + `specs/DeviaTDD-architecture.md` §5.2) updated to document the directive.
...
- **Issue id labels now emit per-epic compound form (`<epic-prefix>-<ordinal>`) for new issues in numbered epics.** New issues in epic `002-…` emit `002-001`, `002-002`, etc.; the adhoc bucket and bootstrap contexts fall back to the legacy global counter `ISS-NNN`. Legacy `ISS-NNN` rows in `specs/issues.jsonl` resolve unchanged via the grandfather contract — the resolve layer is format-agnostic. `_compute_next_issue_id(ledger_path, *, epic_slug=None)` (`src/deviate/cli/macro.py`) gained the per-epic branch with interpretation #2 (legacy `ISS-NNN` rows from the same epic count toward the ordinal so an epic that shipped 19 legacy issues continues at `002-020`); the `shard pre` caller at `cli/macro.py:828` passes `epic_slug=epic_slug`. Pinned by `tests/test_cli/test_macro_contracts.py::TestMacroContracts::test_compute_next_issue_id_*` (5 cases: empty-ledger emission, monotonicity across format boundaries, adhoc/empty-epic-slug fallback, and resolve-layer grandfather) and `::test_shard_pre_emits_per_epic_next_issue_id` (integration pin via `shard pre` contract emit). Specs (`specs/constitution.md` Append-Only Ledger Protocol, `specs/DeviaTDD-api.md` CLI argument examples and IssueRecord schema) updated to match the new format and grandfather contract.
### Fixed
