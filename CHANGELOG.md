...
### Fixed
- **`deviate meso run` now fails fast with `PLAN_NOT_WRITTEN` / `TASKS_NOT_WRITTEN` when an agent returns `status: PASS` without producing its phase artifact.** New `_enforce_phase_artifact` helper (`src/deviate/cli/meso.py`) runs between `_invoke_agent_phase` and `_plan_post` / `_tasks_post` in `_meso_run`, closing the false-success validation gap where the orchestrator trusted the manifest status without verifying the agent wrote `plan.md` / `tasks.md`. The agent-side-of-trust stays where it should (the manifest); the orchestrator now owns the *delivery* contract. New regression tests in `tests/test_meso/test_meso_orchestration.py::TestMesoArtifactEnforcement` (`test_plan_not_written_when_agent_returns_pass`, `test_tasks_not_written_when_agent_returns_pass`) let the post hooks run unmocked so they trip the new guard first — the existing pipeline tests mock both sides and could not catch this regression. Test helper `_setup_minimal_workspace` gains `seed_plan` / `seed_tasks` / `seed_slug` flags so regression tests exercise the missing-artifact path without an `os.remove` dance.
