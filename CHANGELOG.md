...
## [Unreleased]
### Fixed
- **Tome docs site broken under `mise docs`**: `astro dev` crashed with `InvalidContentEntryDataError` because every page that was the first or last in its theme carried `prev: null` / `next: null` per the IA contract, but Starlight's `docsSchema()` declares those fields as `boolean | string | {link, label}` and rejects `null`. Bulk-rewrote 38 pages to use `prev: false` / `next: false` instead (semantically equivalent — both mean "no link"). Aligned the writer and verifier contracts: `src/deviate/prompts/commands/tome-write-{tutorial,how-to,reference,explanation}.md` now emit `false`, `src/deviate/prompts/commands/tome-verify-docs.md` accepts both `null` and `false` as "no link" in its IA reconciliation, and `specs/_product/architecture.md` §3.2/§3.3 document the `false` sentinel and the rationale (Starlight's docsSchema() union; rejecting `null` is a hard constraint of the upstream schema, not a Tome-side policy choice).
