...
### Changed
- **Paused Herdr pane on close so failure output stays visible (emit order corrected).** `with_herdr_status` (`src/deviate/core/herdr.py`) now emits the terminal `idle` / `blocked` envelope FIRST, then calls `pause_for_close()`. The function blocks on stdin until the operator presses Enter, gated on `HERDR_ENV=1`, `sys.stdin.isatty()`, `TERM != "dumb"`, and `HERDR_DEVIATE_NO_PAUSE != "1"`. Corrected from the previous order (pause before emit), which could strand Herdr's `(source, agent)` authority in `working` if the operator pressed Enter / Ctrl-C / EOF at the moment Herdr was processing the parent's exit. Without the pause, the Herdr-tracked pane collapses to the shell prompt the instant the process exits and any failure output scrolls off before the operator can react. EOF / Ctrl-D / Ctrl-C return silently without hanging. Use `HERDR_DEVIATE_NO_PAUSE=1` to opt out (e.g. for non-interactive launches that accidentally inherit the Herdr environment).
...
- **`deviate micro run` cycle loop honors JUDGE's forward-route verdict and stops looping GREEN↔JUDGE on RED-only slices.** The TDD cycle (`_run_tdd_cycle` in `src/deviate/cli/micro.py`) previously drove an infinite GREEN↔JUDGE loop when GREEN returned `status: FAILURE` on a task whose slice had no production-code path (e.g. TSK-009-07's RED-only KCH-fabricated fixture + downstream MCP integration test). JUDGE's `next_action: proceed_to_refactor_no_diff` cleared `session.train_feedback` and `session.judge_rejected`, but the cycle's post-JUDGE check `if session.judge_rejected or session.train_feedback or green_tests_failed:` still found `green_tests_failed=True` (the snapshot of `session.train_feedback and session.current_phase == "GREEN"` captured right after `_run_green_phase` returned on the failed run) — so the cycle re-entered the loop, re-ran GREEN, and re-failed identically. The runner now exits the TRAIN retry loop when JUDGE returns a forward-route verdict via the new sibling check `if session.pending_judge_action in {"continue_refactor", "proceed_to_refactor_no_diff", "skip_refactor"}:` immediately after `_run_judge_phase` returns; that check sets `judge_passed = True` and `break`s out of `while not judge_passed`. `green_tests_failed` is preserved as a sticky flag in the existing post-JUDGE disjunction so the existing `test_micro_green_train_feedback_still_retries_then_exhausts` regression guard (TSK-004-14: GREEN tests keep failing → JUDGE returns a legacy pass → runner must still retry GREEN with `<train_feedback>`) keeps passing — the snapshot is captured at the top of each cycle iteration and survives JUDGE's clear of `session.train_feedback`. Two regression tests: `test_micro_green_mechanical_failure_with_no_diff_routes_to_refactor` (TSK-009-07: `phases = ['RED', 'GREEN', 'JUDGE']`, `mock_agent.call_count == 3`, forward-route exits the loop after one JUDGE call) and the existing `test_micro_green_train_feedback_still_retries_then_exhausts` (TSK-004-14: bare train_feedback must keep the TRAIN retry loop alive). (`src/deviate/cli/micro.py`, `tests/test_micro/test_orchestration.py`.)
