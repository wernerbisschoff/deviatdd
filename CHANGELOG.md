...
### Removed
- **Tree-sitter AST/structural analysis has been removed entirely.** Deleted `src/deviate/core/treesitter/` (parser factory + 21 `.scm` query files), `tests/core/test_treesitter.py`, and all `tree-sitter*` runtime dependencies from `pyproject.toml`. Rewrote `_check_return_type_mismatch` in `src/deviate/cli/micro.py` to use stdlib `ast` (Python-only return-type annotation checks); the dead-code / duplicate-block / cyclomatic-complexity warnings were deleted because they were never wired to any decision. Dropped the `extract_file_structure` / `extract_changed_symbols` contracts from `deviate plan pre` and `deviate review pre`; the review contract no longer emits `structured_diff` or `structured_diff_markdown` and the plan contract no longer emits `file_structure`. ISS-ADH-008 (AST/Structural Analysis) is **REVOKED** — see `specs/adhoc/issues/008-ast-phase-prioritization.md` for the historical spec. **Why now:** the 2.6.0 JUDGE-phase and the 2.6.0 review-phase fixes both papered over the same `tree_sitter_rust._binding` SIGSEGV by banning individual languages; the cleaner cure is to drop the library entirely. The 3rd SIGSEGV that hit `deviate review pre` against a Rust test file in the Gloss 011 worktree (CHANGELOG `### Fixed` entry, same release) was the trigger. **Migration:** structured-diff / file-structure analyses were documented as optional inputs in the review and plan prompts, so the agent dispatch falls back to the raw text diff (the existing source of truth). Specs updated: `DeviaTDD-api.md` `_plan_pre` / `walkthrough pre` contract fields, `DeviaTDD-architecture.md` Plan and Meso Layer sections.
- **Herdr lifecycle reporting and the end-of-process `Press Enter to close this pane` prompt have been removed.** Deleted `src/deviate/core/herdr.py`, `_tracked_run_command` in `src/deviate/main.py`, and the `with_herdr_status` wrapping around Typer dispatch. `app()` now calls `cli()` directly for every argv; tracked commands (`deviate run`, `deviate meso run`, `deviate micro run`) no longer report `working`/`idle`/`blocked` to a Herdr pane socket and no longer block on stdin waiting for the operator to press Enter. The prompt no longer fires anywhere — every `deviate` invocation now terminates normally regardless of any inherited `HERDR_*` environment variables from a parent agent or pane. Regression coverage: `tests/test_cli/test_main_entrypoint.py`.
