...
- **Phase prompts (`/deviate-prd`, `/deviate-plan`, `/deviate-flows`, `/deviate-architecture`, `/deviate-research`) no longer auto-invoke HTML authorship.** Each phase prompt's inline "HTML Artifact" step has been slimmed to a one-line optional pointer to `/deviate-html <phase>` instead of an imperative "run `deviate html <phase>` and author the body now." The HTML counterpart is now an explicit user decision — the user picks when to ship the HTML review surface (typically end-of-session, or per-phase immediately after the markdown lands). The architecture prompt's atomic-commit hint has moved entirely into `/deviate-html`'s STEP_5 (single source of truth, no duplication); the research prompt preserves the conditional gate ("only if `<data_model_target>` was updated this invocation") so re-authoring an unchanged domain-model does not burn work.
