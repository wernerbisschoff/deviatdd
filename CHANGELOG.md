...
### Changed
- **Paused Herdr pane on close so failure output stays visible.** After `with_herdr_status` runs the callback, it now calls `pause_for_close()` BEFORE emitting the terminal `idle`/`blocked` report. The function blocks on stdin until the operator presses Enter, gated on `HERDR_ENV=1`, `sys.stdin.isatty()`, `TERM != "dumb"`, and `HERDR_DEVIATE_NO_PAUSE != "1"`. Without it, the Herdr-tracked pane collapses to the shell prompt the instant the process exits and any failure output scrolls off before the operator can react. EOF / Ctrl-D / Ctrl-C return silently without hanging. Use `HERDR_DEVIATE_NO_PAUSE=1` to opt out (e.g. for non-interactive launches that accidentally inherit the Herdr environment).
